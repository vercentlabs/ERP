...
import type { DocumentIntelligenceCreateInput, DocumentIntelligenceListRequest, DocumentIntelligenceRecord, DocumentIntelligenceUpdateInput } from "./types";

const records = new Map<string, DocumentIntelligenceRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const documentIntelligenceRepository = {
  async list(request: DocumentIntelligenceListRequest): Promise<PageResult<DocumentIntelligenceRecord>> {
  ...
  async create(input: DocumentIntelligenceCreateInput, actorId = "system"): Promise<DocumentIntelligenceRecord> {
    const timestamp = now();
  ...
  async update(tenantId: string, id: string, input: DocumentIntelligenceUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: DocumentIntelligenceRecord = {
      ...current,
      ...input,
  ...
  async save(record: DocumentIntelligenceRecord) {
    records.set(record.id, record);
    return record;
  },
};
