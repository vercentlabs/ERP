...
import type { AnomalyDetectionCreateInput, AnomalyDetectionListRequest, AnomalyDetectionRecord, AnomalyDetectionUpdateInput } from "./types";

const records = new Map<string, AnomalyDetectionRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const anomalyDetectionRepository = {
  async list(request: AnomalyDetectionListRequest): Promise<PageResult<AnomalyDetectionRecord>> {
  ...
  async create(input: AnomalyDetectionCreateInput, actorId = "system"): Promise<AnomalyDetectionRecord> {
    const timestamp = now();
    const record: AnomalyDetectionRecord = {
      id: createId(),
  ...
  async update(tenantId: string, id: string, input: AnomalyDetectionUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: AnomalyDetectionRecord = {
      ...current,
      ...input,
  ...
  async save(record: AnomalyDetectionRecord) {
    records.set(record.id, record);
    return record;
  },
};
