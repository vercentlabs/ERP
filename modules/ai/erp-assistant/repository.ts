...
import type { ErpAssistantCreateInput, ErpAssistantListRequest, ErpAssistantRecord, ErpAssistantUpdateInput } from "./types";

const records = new Map<string, ErpAssistantRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const erpAssistantRepository = {
  async list(request: ErpAssistantListRequest): Promise<PageResult<ErpAssistantRecord>> {
  ...
  async create(input: ErpAssistantCreateInput, actorId = "system"): Promise<ErpAssistantRecord> {
    const timestamp = now();
    const record: ErpAssistantRecord = {
      id: createId(),
  ...
  async update(tenantId: string, id: string, input: ErpAssistantUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: ErpAssistantRecord = {
      ...current,
      ...input,
  ...
  async save(record: ErpAssistantRecord) {
    records.set(record.id, record);
    return record;
  },
};
