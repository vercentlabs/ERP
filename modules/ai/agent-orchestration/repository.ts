...
import type { AgentOrchestrationCreateInput, AgentOrchestrationListRequest, AgentOrchestrationRecord, AgentOrchestrationUpdateInput } from "./types";

const records = new Map<string, AgentOrchestrationRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const agentOrchestrationRepository = {
  async list(request: AgentOrchestrationListRequest): Promise<PageResult<AgentOrchestrationRecord>> {
  ...
  async create(input: AgentOrchestrationCreateInput, actorId = "system"): Promise<AgentOrchestrationRecord> {
    const timestamp = now();
    const record: AgentOrchestrationRecord = {
      id: createId(),
  ...
  async update(tenantId: string, id: string, input: AgentOrchestrationUpdateInput, actorId = "system") {
  ...
  async save(record: AgentOrchestrationRecord) {
    records.set(record.id, record);
    return record;
  },
};
