...
import type { ProfitCentersCreateInput, ProfitCentersListRequest, ProfitCentersRecord, ProfitCentersUpdateInput } from "./types";

const records = new Map<string, ProfitCentersRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const profitCentersRepository = {
  async list(request: ProfitCentersListRequest): Promise<PageResult<ProfitCentersRecord>> {
  ...
  async create(input: ProfitCentersCreateInput, actorId = "system"): Promise<ProfitCentersRecord> {
    const timestamp = now();
    const record: ProfitCentersRecord = {
      id: createId(),
  ...
  async update(tenantId: string, id: string, input: ProfitCentersUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: ProfitCentersRecord = {
      ...current,
      ...input,
  ...
  async save(record: ProfitCentersRecord) {
    records.set(record.id, record);
    return record;
  },
};
