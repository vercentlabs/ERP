...
import type { JournalsCreateInput, JournalsListRequest, JournalsRecord, JournalsUpdateInput } from "./types";

const records = new Map<string, JournalsRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const journalsRepository = {
  async list(request: JournalsListRequest): Promise<PageResult<JournalsRecord>> {
  ...
  async create(input: JournalsCreateInput, actorId = "system"): Promise<JournalsRecord> {
    const timestamp = now();
    const record: JournalsRecord = {
      id: createId(),
  ...
  async update(tenantId: string, id: string, input: JournalsUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: JournalsRecord = {
      ...current,
      ...input,
  ...
  async save(record: JournalsRecord) {
    records.set(record.id, record);
    return record;
  },
};
