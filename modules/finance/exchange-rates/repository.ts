...
import type { ExchangeRatesCreateInput, ExchangeRatesListRequest, ExchangeRatesRecord, ExchangeRatesUpdateInput } from "./types";

const records = new Map<string, ExchangeRatesRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const exchangeRatesRepository = {
  async list(request: ExchangeRatesListRequest): Promise<PageResult<ExchangeRatesRecord>> {
  ...
  async create(input: ExchangeRatesCreateInput, actorId = "system"): Promise<ExchangeRatesRecord> {
    const timestamp = now();
    const record: ExchangeRatesRecord = {
      id: createId(),
  ...
  async update(tenantId: string, id: string, input: ExchangeRatesUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: ExchangeRatesRecord = {
      ...current,
      ...input,
  ...
  async save(record: ExchangeRatesRecord) {
    records.set(record.id, record);
    return record;
  },
};
