...
import type { FiscalYearsCreateInput, FiscalYearsListRequest, FiscalYearsRecord, FiscalYearsUpdateInput } from "./types";

const records = new Map<string, FiscalYearsRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const fiscalYearsRepository = {
  async list(request: FiscalYearsListRequest): Promise<PageResult<FiscalYearsRecord>> {
  ...
  async create(input: FiscalYearsCreateInput, actorId = "system"): Promise<FiscalYearsRecord> {
    const timestamp = now();
    const record: FiscalYearsRecord = {
      id: createId(),
  ...
  async update(tenantId: string, id: string, input: FiscalYearsUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: FiscalYearsRecord = {
      ...current,
      ...input,
  ...
  async save(record: FiscalYearsRecord) {
    records.set(record.id, record);
    return record;
  },
};
