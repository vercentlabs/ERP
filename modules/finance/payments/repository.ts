...
import type { PaymentsCreateInput, PaymentsListRequest, PaymentsRecord, PaymentsUpdateInput } from "./types";

const records = new Map<string, PaymentsRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const paymentsRepository = {
  async list(request: PaymentsListRequest): Promise<PageResult<PaymentsRecord>> {
  ...
  async create(input: PaymentsCreateInput, actorId = "system"): Promise<PaymentsRecord> {
    const timestamp = now();
  ...
  async update(tenantId: string, id: string, input: PaymentsUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: PaymentsRecord = {
      ...current,
      ...input,
  ...
  async save(record: PaymentsRecord) {
    records.set(record.id, record);
    return record;
  },
};
