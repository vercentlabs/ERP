...
import type { ReceiptsCreateInput, ReceiptsListRequest, ReceiptsRecord, ReceiptsUpdateInput } from "./types";

const records = new Map<string, ReceiptsRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const receiptsRepository = {
  async list(request: ReceiptsListRequest): Promise<PageResult<ReceiptsRecord>> {
  ...
  async create(input: ReceiptsCreateInput, actorId = "system"): Promise<ReceiptsRecord> {
    const timestamp = now();
    const record: ReceiptsRecord = {
      id: createId(),
  ...
  async update(tenantId: string, id: string, input: ReceiptsUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: ReceiptsRecord = {
      ...current,
      ...input,
  ...
  async save(record: ReceiptsRecord) {
    records.set(record.id, record);
    return record;
  },
};
