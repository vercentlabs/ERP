...
import type { BankAccountsCreateInput, BankAccountsListRequest, BankAccountsRecord, BankAccountsUpdateInput } from "./types";

const records = new Map<string, BankAccountsRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const bankAccountsRepository = {
  async list(request: BankAccountsListRequest): Promise<PageResult<BankAccountsRecord>> {
  ...
  async create(input: BankAccountsCreateInput, actorId = "system"): Promise<BankAccountsRecord> {
    const timestamp = now();
    const record: BankAccountsRecord = {
      id: createId(),
  ...
  async update(tenantId: string, id: string, input: BankAccountsUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: BankAccountsRecord = {
      ...current,
      ...input,
  ...
  async save(record: BankAccountsRecord) {
    records.set(record.id, record);
    return record;
  },
};
