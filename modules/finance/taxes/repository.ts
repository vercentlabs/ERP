...
import type { TaxesCreateInput, TaxesListRequest, TaxesRecord, TaxesUpdateInput } from "./types";

const records = new Map<string, TaxesRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const taxesRepository = {
  async list(request: TaxesListRequest): Promise<PageResult<TaxesRecord>> {
  ...
  async create(input: TaxesCreateInput, actorId = "system"): Promise<TaxesRecord> {
    const timestamp = now();
  ...
  async update(tenantId: string, id: string, input: TaxesUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: TaxesRecord = {
      ...current,
      ...input,
