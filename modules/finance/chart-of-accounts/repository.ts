...
import type { ChartOfAccountsCreateInput, ChartOfAccountsListRequest, ChartOfAccountsRecord, ChartOfAccountsUpdateInput } from "./types";

const records = new Map<string, ChartOfAccountsRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const chartOfAccountsRepository = {
  async list(request: ChartOfAccountsListRequest): Promise<PageResult<ChartOfAccountsRecord>> {
  ...
  async create(input: ChartOfAccountsCreateInput, actorId = "system"): Promise<ChartOfAccountsRecord> {
    const timestamp = now();
    const record: ChartOfAccountsRecord = {
      id: createId(),
  ...
  async update(tenantId: string, id: string, input: ChartOfAccountsUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: ChartOfAccountsRecord = {
      ...current,
      ...input,
  ...
  async save(record: ChartOfAccountsRecord) {
    records.set(record.id, record);
    return record;
  },
};
