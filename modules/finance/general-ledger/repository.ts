...
import type { GeneralLedgerCreateInput, GeneralLedgerListRequest, GeneralLedgerRecord, GeneralLedgerUpdateInput } from "./types";

const records = new Map<string, GeneralLedgerRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const generalLedgerRepository = {
  async list(request: GeneralLedgerListRequest): Promise<PageResult<GeneralLedgerRecord>> {
  ...
  async create(input: GeneralLedgerCreateInput, actorId = "system"): Promise<GeneralLedgerRecord> {
    const timestamp = now();
    const record: GeneralLedgerRecord = {
      id: createId(),
  ...
  async update(tenantId: string, id: string, input: GeneralLedgerUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: GeneralLedgerRecord = {
      ...current,
      ...input,
  ...
  async save(record: GeneralLedgerRecord) {
    records.set(record.id, record);
    return record;
  },
};
