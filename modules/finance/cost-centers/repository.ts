...
import type { CostCentersCreateInput, CostCentersListRequest, CostCentersRecord, CostCentersUpdateInput } from "./types";

const records = new Map<string, CostCentersRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const costCentersRepository = {
  async list(request: CostCentersListRequest): Promise<PageResult<CostCentersRecord>> {
  ...
  async create(input: CostCentersCreateInput, actorId = "system"): Promise<CostCentersRecord> {
    const timestamp = now();
    const record: CostCentersRecord = {
      id: createId(),
  ...
  async update(tenantId: string, id: string, input: CostCentersUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: CostCentersRecord = {
      ...current,
      ...input,
  ...
  async save(record: CostCentersRecord) {
    records.set(record.id, record);
    return record;
  },
};
