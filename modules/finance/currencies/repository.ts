...
import type { CurrenciesCreateInput, CurrenciesListRequest, CurrenciesRecord, CurrenciesUpdateInput } from "./types";

const records = new Map<string, CurrenciesRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const currenciesRepository = {
  async list(request: CurrenciesListRequest): Promise<PageResult<CurrenciesRecord>> {
  ...
  async create(input: CurrenciesCreateInput, actorId = "system"): Promise<CurrenciesRecord> {
    const timestamp = now();
    const record: CurrenciesRecord = {
      id: createId(),
  ...
  async update(tenantId: string, id: string, input: CurrenciesUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: CurrenciesRecord = {
      ...current,
      ...input,
  ...
  async save(record: CurrenciesRecord) {
    records.set(record.id, record);
    return record;
  },
};
