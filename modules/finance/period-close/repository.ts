...
import type { PeriodCloseCreateInput, PeriodCloseListRequest, PeriodCloseRecord, PeriodCloseUpdateInput } from "./types";

const records = new Map<string, PeriodCloseRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const periodCloseRepository = {
  async list(request: PeriodCloseListRequest): Promise<PageResult<PeriodCloseRecord>> {
  ...
  async create(input: PeriodCloseCreateInput, actorId = "system"): Promise<PeriodCloseRecord> {
    const timestamp = now();
  ...
  async update(tenantId: string, id: string, input: PeriodCloseUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: PeriodCloseRecord = {
      ...current,
      ...input,
  ...
  async save(record: PeriodCloseRecord) {
    records.set(record.id, record);
    return record;
  },
};
