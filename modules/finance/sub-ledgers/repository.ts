...
import type { SubLedgersCreateInput, SubLedgersListRequest, SubLedgersRecord, SubLedgersUpdateInput } from "./types";

const records = new Map<string, SubLedgersRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const subLedgersRepository = {
  async list(request: SubLedgersListRequest): Promise<PageResult<SubLedgersRecord>> {
  ...
  async create(input: SubLedgersCreateInput, actorId = "system"): Promise<SubLedgersRecord> {
    const timestamp = now();
    const record: SubLedgersRecord = {
      id: createId(),
  ...
  async update(tenantId: string, id: string, input: SubLedgersUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: SubLedgersRecord = {
      ...current,
      ...input,
  ...
  async save(record: SubLedgersRecord) {
    records.set(record.id, record);
    return record;
  },
};
