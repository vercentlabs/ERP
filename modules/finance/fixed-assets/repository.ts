...
import type { FixedAssetsCreateInput, FixedAssetsListRequest, FixedAssetsRecord, FixedAssetsUpdateInput } from "./types";

const records = new Map<string, FixedAssetsRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const fixedAssetsRepository = {
  async list(request: FixedAssetsListRequest): Promise<PageResult<FixedAssetsRecord>> {
  ...
  async create(input: FixedAssetsCreateInput, actorId = "system"): Promise<FixedAssetsRecord> {
    const timestamp = now();
    const record: FixedAssetsRecord = {
      id: createId(),
  ...
  async update(tenantId: string, id: string, input: FixedAssetsUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: FixedAssetsRecord = {
      ...current,
      ...input,
  ...
  async save(record: FixedAssetsRecord) {
    records.set(record.id, record);
    return record;
  },
};
