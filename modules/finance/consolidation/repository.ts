...
import type { ConsolidationCreateInput, ConsolidationListRequest, ConsolidationRecord, ConsolidationUpdateInput } from "./types";
...
export const consolidationRepository = {
  async list(request: ConsolidationListRequest): Promise<PageResult<ConsolidationRecord>> {
  ...
  async create(input: ConsolidationCreateInput, actorId = "system"): Promise<ConsolidationRecord> {
    const timestamp = now();
  ...
  async update(tenantId: string, id: string, input: ConsolidationUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: ConsolidationRecord = {
      ...current,
      ...input,
  ...
  async save(record: ConsolidationRecord) {
    records.set(record.id, record);
    return record;
  },
};
