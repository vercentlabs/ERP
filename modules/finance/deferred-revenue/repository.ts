...
import type { DeferredRevenueCreateInput, DeferredRevenueListRequest, DeferredRevenueRecord, DeferredRevenueUpdateInput } from "./types";

const records = new Map<string, DeferredRevenueRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const deferredRevenueRepository = {
  async list(request: DeferredRevenueListRequest): Promise<PageResult<DeferredRevenueRecord>> {
  ...
  async create(input: DeferredRevenueCreateInput, actorId = "system"): Promise<DeferredRevenueRecord> {
    const timestamp = now();
  ...
  async update(tenantId: string, id: string, input: DeferredRevenueUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: DeferredRevenueRecord = {
      ...current,
      ...input,
  ...
  async save(record: DeferredRevenueRecord) {
    records.set(record.id, record);
    return record;
  },
};
