...
import type { BankReconciliationCreateInput, BankReconciliationListRequest, BankReconciliationRecord, BankReconciliationUpdateInput } from "./types";

const records = new Map<string, BankReconciliationRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const bankReconciliationRepository = {
  async list(request: BankReconciliationListRequest): Promise<PageResult<BankReconciliationRecord>> {
  ...
  async create(input: BankReconciliationCreateInput, actorId = "system"): Promise<BankReconciliationRecord> {
    const timestamp = now();
  ...
  async update(tenantId: string, id: string, input: BankReconciliationUpdateInput, actorId = "system") {
  ...
  async save(record: BankReconciliationRecord) {
    records.set(record.id, record);
    return record;
  },
};
