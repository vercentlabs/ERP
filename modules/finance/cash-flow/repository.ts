...
import type { CashFlowCreateInput, CashFlowListRequest, CashFlowRecord, CashFlowUpdateInput } from "./types";

const records = new Map<string, CashFlowRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const cashFlowRepository = {
  async list(request: CashFlowListRequest): Promise<PageResult<CashFlowRecord>> {
  ...
  async create(input: CashFlowCreateInput, actorId = "system"): Promise<CashFlowRecord> {
    const timestamp = now();
  ...
  async update(tenantId: string, id: string, input: CashFlowUpdateInput, actorId = "system") {
  ...
  async save(record: CashFlowRecord) {
    records.set(record.id, record);
    return record;
  },
};
