...
import type { AccountsReceivableCreateInput, AccountsReceivableListRequest, AccountsReceivableRecord, AccountsReceivableUpdateInput } from "./types";

const records = new Map<string, AccountsReceivableRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const accountsReceivableRepository = {
  async list(request: AccountsReceivableListRequest): Promise<PageResult<AccountsReceivableRecord>> {
  ...
  async create(input: AccountsReceivableCreateInput, actorId = "system"): Promise<AccountsReceivableRecord> {
    const timestamp = now();
    const record: AccountsReceivableRecord = {
      id: createId(),
  ...
  async update(tenantId: string, id: string, input: AccountsReceivableUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: AccountsReceivableRecord = {
      ...current,
      ...input,
  ...
  async save(record: AccountsReceivableRecord) {
    records.set(record.id, record);
    return record;
  },
};
