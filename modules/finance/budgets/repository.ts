...
import type { BudgetsCreateInput, BudgetsListRequest, BudgetsRecord, BudgetsUpdateInput } from "./types";

const records = new Map<string, BudgetsRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const budgetsRepository = {
  async list(request: BudgetsListRequest): Promise<PageResult<BudgetsRecord>> {
  ...
  async create(input: BudgetsCreateInput, actorId = "system"): Promise<BudgetsRecord> {
    const timestamp = now();
    const record: BudgetsRecord = {
      id: createId(),
  ...
  async update(tenantId: string, id: string, input: BudgetsUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: BudgetsRecord = {
      ...current,
      ...input,
  ...
  async save(record: BudgetsRecord) {
    records.set(record.id, record);
    return record;
  },
};
