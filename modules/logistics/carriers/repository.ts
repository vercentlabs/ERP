...
import type { CarriersCreateInput, CarriersListRequest, CarriersRecord, CarriersUpdateInput } from "./types";

const records = new Map<string, CarriersRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const carriersRepository = {
  async list(request: CarriersListRequest): Promise<PageResult<CarriersRecord>> {
  ...
  async create(input: CarriersCreateInput, actorId = "system"): Promise<CarriersRecord> {
    const timestamp = now();
    const record: CarriersRecord = {
      id: createId(),
  ...
  async update(tenantId: string, id: string, input: CarriersUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: CarriersRecord = {
      ...current,
      ...input,
  ...
  async save(record: CarriersRecord) {
    records.set(record.id, record);
    return record;
  },
};
