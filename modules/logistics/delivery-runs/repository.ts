...
import type { DeliveryRunsCreateInput, DeliveryRunsListRequest, DeliveryRunsRecord, DeliveryRunsUpdateInput } from "./types";

const records = new Map<string, DeliveryRunsRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const deliveryRunsRepository = {
  async list(request: DeliveryRunsListRequest): Promise<PageResult<DeliveryRunsRecord>> {
  ...
  async create(input: DeliveryRunsCreateInput, actorId = "system"): Promise<DeliveryRunsRecord> {
    const timestamp = now();
    const record: DeliveryRunsRecord = {
      id: createId(),
  ...
  async update(tenantId: string, id: string, input: DeliveryRunsUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: DeliveryRunsRecord = {
      ...current,
      ...input,
  ...
  async save(record: DeliveryRunsRecord) {
    records.set(record.id, record);
    return record;
  },
};
