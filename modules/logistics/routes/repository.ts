...
import type { RoutesCreateInput, RoutesListRequest, RoutesRecord, RoutesUpdateInput } from "./types";

const records = new Map<string, RoutesRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const routesRepository = {
  async list(request: RoutesListRequest): Promise<PageResult<RoutesRecord>> {
  ...
  async create(input: RoutesCreateInput, actorId = "system"): Promise<RoutesRecord> {
    const timestamp = now();
    const record: RoutesRecord = {
      id: createId(),
  ...
  async update(tenantId: string, id: string, input: RoutesUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: RoutesRecord = {
      ...current,
      ...input,
  ...
  async save(record: RoutesRecord) {
    records.set(record.id, record);
    return record;
  },
};
