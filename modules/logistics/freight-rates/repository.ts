...
import type { FreightRatesCreateInput, FreightRatesListRequest, FreightRatesRecord, FreightRatesUpdateInput } from "./types";

const records = new Map<string, FreightRatesRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const freightRatesRepository = {
  async list(request: FreightRatesListRequest): Promise<PageResult<FreightRatesRecord>> {
  ...
  async create(input: FreightRatesCreateInput, actorId = "system"): Promise<FreightRatesRecord> {
    const timestamp = now();
    const record: FreightRatesRecord = {
      id: createId(),
  ...
  async update(tenantId: string, id: string, input: FreightRatesUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: FreightRatesRecord = {
      ...current,
      ...input,
  ...
  async save(record: FreightRatesRecord) {
    records.set(record.id, record);
    return record;
  },
};
