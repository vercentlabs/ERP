...
import type { ProofOfDeliveryCreateInput, ProofOfDeliveryListRequest, ProofOfDeliveryRecord, ProofOfDeliveryUpdateInput } from "./types";

const records = new Map<string, ProofOfDeliveryRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const proofOfDeliveryRepository = {
  async list(request: ProofOfDeliveryListRequest): Promise<PageResult<ProofOfDeliveryRecord>> {
  ...
  async create(input: ProofOfDeliveryCreateInput, actorId = "system"): Promise<ProofOfDeliveryRecord> {
    const timestamp = now();
  ...
  async update(tenantId: string, id: string, input: ProofOfDeliveryUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: ProofOfDeliveryRecord = {
      ...current,
      ...input,
  ...
  async save(record: ProofOfDeliveryRecord) {
    records.set(record.id, record);
    return record;
  },
};
