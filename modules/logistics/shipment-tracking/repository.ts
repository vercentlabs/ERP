...
import type { ShipmentTrackingCreateInput, ShipmentTrackingListRequest, ShipmentTrackingRecord, ShipmentTrackingUpdateInput } from "./types";

const records = new Map<string, ShipmentTrackingRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const shipmentTrackingRepository = {
  async list(request: ShipmentTrackingListRequest): Promise<PageResult<ShipmentTrackingRecord>> {
  ...
  async create(input: ShipmentTrackingCreateInput, actorId = "system"): Promise<ShipmentTrackingRecord> {
    const timestamp = now();
    const record: ShipmentTrackingRecord = {
      id: createId(),
  ...
  async update(tenantId: string, id: string, input: ShipmentTrackingUpdateInput, actorId = "system") {
  ...
  async save(record: ShipmentTrackingRecord) {
    records.set(record.id, record);
    return record;
  },
};
