...
import type { ShipmentsCreateInput, ShipmentsListRequest, ShipmentsRecord, ShipmentsUpdateInput } from "./types";

const records = new Map<string, ShipmentsRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const shipmentsRepository = {
  async list(request: ShipmentsListRequest): Promise<PageResult<ShipmentsRecord>> {
  ...
  async create(input: ShipmentsCreateInput, actorId = "system"): Promise<ShipmentsRecord> {
    const timestamp = now();
    const record: ShipmentsRecord = {
      id: createId(),
  ...
  async update(tenantId: string, id: string, input: ShipmentsUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: ShipmentsRecord = {
      ...current,
      ...input,
  ...
  async save(record: ShipmentsRecord) {
    records.set(record.id, record);
    return record;
  },
};
