...
import type { PlansCreateInput, PlansListRequest, PlansRecord, PlansUpdateInput } from "./types";

const records = new Map<string, PlansRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const plansRepository = {
  async list(request: PlansListRequest): Promise<PageResult<PlansRecord>> {
  ...
  async create(input: PlansCreateInput, actorId = "system"): Promise<PlansRecord> {
    const timestamp = now();
    const record: PlansRecord = {
      id: createId(),
  ...
  async update(tenantId: string, id: string, input: PlansUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: PlansRecord = {
      ...current,
      ...input,
