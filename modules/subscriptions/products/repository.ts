...
import type { ProductsCreateInput, ProductsListRequest, ProductsRecord, ProductsUpdateInput } from "./types";

const records = new Map<string, ProductsRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const productsRepository = {
  async list(request: ProductsListRequest): Promise<PageResult<ProductsRecord>> {
  ...
  async create(input: ProductsCreateInput, actorId = "system"): Promise<ProductsRecord> {
    const timestamp = now();
    const record: ProductsRecord = {
      id: createId(),
  ...
  async update(tenantId: string, id: string, input: ProductsUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: ProductsRecord = {
      ...current,
      ...input,
  ...
  async save(record: ProductsRecord) {
    records.set(record.id, record);
    return record;
  },
};
