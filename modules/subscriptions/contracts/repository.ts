...
import type { ContractsCreateInput, ContractsListRequest, ContractsRecord, ContractsUpdateInput } from "./types";

const records = new Map<string, ContractsRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const contractsRepository = {
  async list(request: ContractsListRequest): Promise<PageResult<ContractsRecord>> {
  ...
  async create(input: ContractsCreateInput, actorId = "system"): Promise<ContractsRecord> {
    const timestamp = now();
    const record: ContractsRecord = {
      id: createId(),
  ...
  async update(tenantId: string, id: string, input: ContractsUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: ContractsRecord = {
      ...current,
      ...input,
  ...
  async save(record: ContractsRecord) {
    records.set(record.id, record);
    return record;
  },
};
