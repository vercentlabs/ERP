...
import type { MeteredUsageCreateInput, MeteredUsageListRequest, MeteredUsageRecord, MeteredUsageUpdateInput } from "./types";

const records = new Map<string, MeteredUsageRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const meteredUsageRepository = {
  async list(request: MeteredUsageListRequest): Promise<PageResult<MeteredUsageRecord>> {
  ...
  async create(input: MeteredUsageCreateInput, actorId = "system"): Promise<MeteredUsageRecord> {
    const timestamp = now();
    const record: MeteredUsageRecord = {
      id: createId(),
  ...
  async update(tenantId: string, id: string, input: MeteredUsageUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: MeteredUsageRecord = {
      ...current,
      ...input,
  ...
  async save(record: MeteredUsageRecord) {
    records.set(record.id, record);
    return record;
  },
};
