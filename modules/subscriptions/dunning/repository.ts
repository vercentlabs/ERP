...
import type { DunningCreateInput, DunningListRequest, DunningRecord, DunningUpdateInput } from "./types";
...
export const dunningRepository = {
  async list(request: DunningListRequest): Promise<PageResult<DunningRecord>> {
  ...
  async create(input: DunningCreateInput, actorId = "system"): Promise<DunningRecord> {
    const timestamp = now();
  ...
  async update(tenantId: string, id: string, input: DunningUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: DunningRecord = {
      ...current,
      ...input,
  ...
  async save(record: DunningRecord) {
    records.set(record.id, record);
    return record;
  },
};
