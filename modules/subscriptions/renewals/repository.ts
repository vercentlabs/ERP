...
import type { RenewalsCreateInput, RenewalsListRequest, RenewalsRecord, RenewalsUpdateInput } from "./types";

const records = new Map<string, RenewalsRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const renewalsRepository = {
  async list(request: RenewalsListRequest): Promise<PageResult<RenewalsRecord>> {
  ...
  async create(input: RenewalsCreateInput, actorId = "system"): Promise<RenewalsRecord> {
    const timestamp = now();
    const record: RenewalsRecord = {
      id: createId(),
  ...
  async update(tenantId: string, id: string, input: RenewalsUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: RenewalsRecord = {
      ...current,
      ...input,
  ...
  async save(record: RenewalsRecord) {
    records.set(record.id, record);
    return record;
  },
};
