...
import type { EventStreamsCreateInput, EventStreamsListRequest, EventStreamsRecord, EventStreamsUpdateInput } from "./types";

const records = new Map<string, EventStreamsRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const eventStreamsRepository = {
  async list(request: EventStreamsListRequest): Promise<PageResult<EventStreamsRecord>> {
  ...
  async create(input: EventStreamsCreateInput, actorId = "system"): Promise<EventStreamsRecord> {
    const timestamp = now();
    const record: EventStreamsRecord = {
      id: createId(),
  ...
  async update(tenantId: string, id: string, input: EventStreamsUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: EventStreamsRecord = {
      ...current,
      ...input,
  ...
  async save(record: EventStreamsRecord) {
    records.set(record.id, record);
    return record;
  },
};
