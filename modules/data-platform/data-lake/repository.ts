...
import type { DataLakeCreateInput, DataLakeListRequest, DataLakeRecord, DataLakeUpdateInput } from "./types";

const records = new Map<string, DataLakeRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const dataLakeRepository = {
  async list(request: DataLakeListRequest): Promise<PageResult<DataLakeRecord>> {
  ...
  async create(input: DataLakeCreateInput, actorId = "system"): Promise<DataLakeRecord> {
    const timestamp = now();
  ...
  async update(tenantId: string, id: string, input: DataLakeUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: DataLakeRecord = {
      ...current,
      ...input,
  ...
  async save(record: DataLakeRecord) {
    records.set(record.id, record);
    return record;
  },
};
