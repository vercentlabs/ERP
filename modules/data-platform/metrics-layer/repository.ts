...
import type { MetricsLayerCreateInput, MetricsLayerListRequest, MetricsLayerRecord, MetricsLayerUpdateInput } from "./types";

const records = new Map<string, MetricsLayerRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const metricsLayerRepository = {
  async list(request: MetricsLayerListRequest): Promise<PageResult<MetricsLayerRecord>> {
  ...
  async create(input: MetricsLayerCreateInput, actorId = "system"): Promise<MetricsLayerRecord> {
    const timestamp = now();
    const record: MetricsLayerRecord = {
      id: createId(),
  ...
  async update(tenantId: string, id: string, input: MetricsLayerUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: MetricsLayerRecord = {
      ...current,
      ...input,
  ...
  async save(record: MetricsLayerRecord) {
    records.set(record.id, record);
    return record;
  },
};
