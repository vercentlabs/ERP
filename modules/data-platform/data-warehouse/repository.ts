...
import type { DataWarehouseCreateInput, DataWarehouseListRequest, DataWarehouseRecord, DataWarehouseUpdateInput } from "./types";

const records = new Map<string, DataWarehouseRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const dataWarehouseRepository = {
  async list(request: DataWarehouseListRequest): Promise<PageResult<DataWarehouseRecord>> {
  ...
  async create(input: DataWarehouseCreateInput, actorId = "system"): Promise<DataWarehouseRecord> {
    const timestamp = now();
    const record: DataWarehouseRecord = {
      id: createId(),
  ...
  async update(tenantId: string, id: string, input: DataWarehouseUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: DataWarehouseRecord = {
      ...current,
      ...input,
  ...
  async save(record: DataWarehouseRecord) {
    records.set(record.id, record);
    return record;
  },
};
