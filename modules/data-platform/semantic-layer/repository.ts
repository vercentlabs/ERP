...
import type { SemanticLayerCreateInput, SemanticLayerListRequest, SemanticLayerRecord, SemanticLayerUpdateInput } from "./types";

const records = new Map<string, SemanticLayerRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const semanticLayerRepository = {
  async list(request: SemanticLayerListRequest): Promise<PageResult<SemanticLayerRecord>> {
  ...
  async create(input: SemanticLayerCreateInput, actorId = "system"): Promise<SemanticLayerRecord> {
    const timestamp = now();
    const record: SemanticLayerRecord = {
      id: createId(),
  ...
  async update(tenantId: string, id: string, input: SemanticLayerUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: SemanticLayerRecord = {
      ...current,
      ...input,
  ...
  async save(record: SemanticLayerRecord) {
    records.set(record.id, record);
    return record;
  },
};
