...
import type { BiConnectorsCreateInput, BiConnectorsListRequest, BiConnectorsRecord, BiConnectorsUpdateInput } from "./types";

const records = new Map<string, BiConnectorsRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const biConnectorsRepository = {
  async list(request: BiConnectorsListRequest): Promise<PageResult<BiConnectorsRecord>> {
  ...
  async create(input: BiConnectorsCreateInput, actorId = "system"): Promise<BiConnectorsRecord> {
    const timestamp = now();
    const record: BiConnectorsRecord = {
      id: createId(),
  ...
  async update(tenantId: string, id: string, input: BiConnectorsUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: BiConnectorsRecord = {
      ...current,
      ...input,
  ...
  async save(record: BiConnectorsRecord) {
    records.set(record.id, record);
    return record;
  },
};
