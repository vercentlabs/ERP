...
import type { CdcCreateInput, CdcListRequest, CdcRecord, CdcUpdateInput } from "./types";

const records = new Map<string, CdcRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const cdcRepository = {
  async list(request: CdcListRequest): Promise<PageResult<CdcRecord>> {
  ...
  async create(input: CdcCreateInput, actorId = "system"): Promise<CdcRecord> {
    const timestamp = now();
    const record: CdcRecord = {
      id: createId(),
  ...
  async update(tenantId: string, id: string, input: CdcUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: CdcRecord = {
      ...current,
      ...input,
