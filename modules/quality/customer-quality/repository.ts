...
import type { CustomerQualityCreateInput, CustomerQualityListRequest, CustomerQualityRecord, CustomerQualityUpdateInput } from "./types";

const records = new Map<string, CustomerQualityRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const customerQualityRepository = {
  async list(request: CustomerQualityListRequest): Promise<PageResult<CustomerQualityRecord>> {
  ...
  async create(input: CustomerQualityCreateInput, actorId = "system"): Promise<CustomerQualityRecord> {
    const timestamp = now();
  ...
  async update(tenantId: string, id: string, input: CustomerQualityUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: CustomerQualityRecord = {
      ...current,
      ...input,
  ...
  async save(record: CustomerQualityRecord) {
    records.set(record.id, record);
    return record;
  },
};
