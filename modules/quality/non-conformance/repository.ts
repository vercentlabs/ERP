...
import type { NonConformanceCreateInput, NonConformanceListRequest, NonConformanceRecord, NonConformanceUpdateInput } from "./types";

const records = new Map<string, NonConformanceRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const nonConformanceRepository = {
  async list(request: NonConformanceListRequest): Promise<PageResult<NonConformanceRecord>> {
  ...
  async create(input: NonConformanceCreateInput, actorId = "system"): Promise<NonConformanceRecord> {
    const timestamp = now();
  ...
  async update(tenantId: string, id: string, input: NonConformanceUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: NonConformanceRecord = {
      ...current,
      ...input,
  ...
  async save(record: NonConformanceRecord) {
    records.set(record.id, record);
    return record;
  },
};
