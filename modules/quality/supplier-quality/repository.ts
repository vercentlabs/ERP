...
import type { SupplierQualityCreateInput, SupplierQualityListRequest, SupplierQualityRecord, SupplierQualityUpdateInput } from "./types";

const records = new Map<string, SupplierQualityRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const supplierQualityRepository = {
  async list(request: SupplierQualityListRequest): Promise<PageResult<SupplierQualityRecord>> {
  ...
  async create(input: SupplierQualityCreateInput, actorId = "system"): Promise<SupplierQualityRecord> {
    const timestamp = now();
  ...
  async update(tenantId: string, id: string, input: SupplierQualityUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: SupplierQualityRecord = {
      ...current,
      ...input,
  ...
  async save(record: SupplierQualityRecord) {
    records.set(record.id, record);
    return record;
  },
};
