...
import type { MilestonesCreateInput, MilestonesListRequest, MilestonesRecord, MilestonesUpdateInput } from "./types";

const records = new Map<string, MilestonesRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const milestonesRepository = {
  async list(request: MilestonesListRequest): Promise<PageResult<MilestonesRecord>> {
  ...
  async create(input: MilestonesCreateInput, actorId = "system"): Promise<MilestonesRecord> {
    const timestamp = now();
    const record: MilestonesRecord = {
      id: createId(),
  ...
  async update(tenantId: string, id: string, input: MilestonesUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: MilestonesRecord = {
      ...current,
      ...input,
  ...
  async save(record: MilestonesRecord) {
    records.set(record.id, record);
    return record;
  },
};
