...
import type { TimesheetsCreateInput, TimesheetsListRequest, TimesheetsRecord, TimesheetsUpdateInput } from "./types";

const records = new Map<string, TimesheetsRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const timesheetsRepository = {
  async list(request: TimesheetsListRequest): Promise<PageResult<TimesheetsRecord>> {
  ...
  async create(input: TimesheetsCreateInput, actorId = "system"): Promise<TimesheetsRecord> {
    const timestamp = now();
    const record: TimesheetsRecord = {
      id: createId(),
  ...
  async update(tenantId: string, id: string, input: TimesheetsUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: TimesheetsRecord = {
      ...current,
      ...input,
  ...
  async save(record: TimesheetsRecord) {
    records.set(record.id, record);
    return record;
  },
};
