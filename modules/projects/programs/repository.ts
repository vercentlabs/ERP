...
import type { ProgramsCreateInput, ProgramsListRequest, ProgramsRecord, ProgramsUpdateInput } from "./types";

const records = new Map<string, ProgramsRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const programsRepository = {
  async list(request: ProgramsListRequest): Promise<PageResult<ProgramsRecord>> {
  ...
  async create(input: ProgramsCreateInput, actorId = "system"): Promise<ProgramsRecord> {
    const timestamp = now();
    const record: ProgramsRecord = {
      id: createId(),
  ...
  async update(tenantId: string, id: string, input: ProgramsUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: ProgramsRecord = {
      ...current,
      ...input,
  ...
  async save(record: ProgramsRecord) {
    records.set(record.id, record);
    return record;
  },
};
