...
import type { ProjectBillingCreateInput, ProjectBillingListRequest, ProjectBillingRecord, ProjectBillingUpdateInput } from "./types";

const records = new Map<string, ProjectBillingRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const projectBillingRepository = {
  async list(request: ProjectBillingListRequest): Promise<PageResult<ProjectBillingRecord>> {
  ...
  async create(input: ProjectBillingCreateInput, actorId = "system"): Promise<ProjectBillingRecord> {
    const timestamp = now();
    const record: ProjectBillingRecord = {
      id: createId(),
  ...
  async update(tenantId: string, id: string, input: ProjectBillingUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: ProjectBillingRecord = {
      ...current,
      ...input,
  ...
  async save(record: ProjectBillingRecord) {
    records.set(record.id, record);
    return record;
  },
};
