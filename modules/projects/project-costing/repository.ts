...
import type { ProjectCostingCreateInput, ProjectCostingListRequest, ProjectCostingRecord, ProjectCostingUpdateInput } from "./types";

const records = new Map<string, ProjectCostingRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const projectCostingRepository = {
  async list(request: ProjectCostingListRequest): Promise<PageResult<ProjectCostingRecord>> {
  ...
  async create(input: ProjectCostingCreateInput, actorId = "system"): Promise<ProjectCostingRecord> {
    const timestamp = now();
    const record: ProjectCostingRecord = {
      id: createId(),
  ...
  async update(tenantId: string, id: string, input: ProjectCostingUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: ProjectCostingRecord = {
      ...current,
      ...input,
  ...
  async save(record: ProjectCostingRecord) {
    records.set(record.id, record);
    return record;
  },
};
