...
import type { ProfitabilityCreateInput, ProfitabilityListRequest, ProfitabilityRecord, ProfitabilityUpdateInput } from "./types";

const records = new Map<string, ProfitabilityRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const profitabilityRepository = {
  async list(request: ProfitabilityListRequest): Promise<PageResult<ProfitabilityRecord>> {
  ...
  async create(input: ProfitabilityCreateInput, actorId = "system"): Promise<ProfitabilityRecord> {
    const timestamp = now();
    const record: ProfitabilityRecord = {
      id: createId(),
  ...
  async update(tenantId: string, id: string, input: ProfitabilityUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: ProfitabilityRecord = {
      ...current,
      ...input,
  ...
  async save(record: ProfitabilityRecord) {
    records.set(record.id, record);
    return record;
  },
};
