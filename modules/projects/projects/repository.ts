...
import type { ProjectsCreateInput, ProjectsListRequest, ProjectsRecord, ProjectsUpdateInput } from "./types";

const records = new Map<string, ProjectsRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const projectsRepository = {
  async list(request: ProjectsListRequest): Promise<PageResult<ProjectsRecord>> {
  ...
  async create(input: ProjectsCreateInput, actorId = "system"): Promise<ProjectsRecord> {
    const timestamp = now();
  ...
  async update(tenantId: string, id: string, input: ProjectsUpdateInput, actorId = "system") {
  ...
  async save(record: ProjectsRecord) {
    records.set(record.id, record);
    return record;
  },
};
