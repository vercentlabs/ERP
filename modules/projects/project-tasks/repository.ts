...
import type { ProjectTasksCreateInput, ProjectTasksListRequest, ProjectTasksRecord, ProjectTasksUpdateInput } from "./types";

const records = new Map<string, ProjectTasksRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const projectTasksRepository = {
  async list(request: ProjectTasksListRequest): Promise<PageResult<ProjectTasksRecord>> {
  ...
  async create(input: ProjectTasksCreateInput, actorId = "system"): Promise<ProjectTasksRecord> {
    const timestamp = now();
    const record: ProjectTasksRecord = {
      id: createId(),
  ...
  async update(tenantId: string, id: string, input: ProjectTasksUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: ProjectTasksRecord = {
      ...current,
      ...input,
  ...
  async save(record: ProjectTasksRecord) {
    records.set(record.id, record);
    return record;
  },
};
