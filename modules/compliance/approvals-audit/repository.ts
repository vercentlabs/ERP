...
import type { ApprovalsAuditCreateInput, ApprovalsAuditListRequest, ApprovalsAuditRecord, ApprovalsAuditUpdateInput } from "./types";

const records = new Map<string, ApprovalsAuditRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const approvalsAuditRepository = {
  async list(request: ApprovalsAuditListRequest): Promise<PageResult<ApprovalsAuditRecord>> {
  ...
  async create(input: ApprovalsAuditCreateInput, actorId = "system"): Promise<ApprovalsAuditRecord> {
    const timestamp = now();
    const record: ApprovalsAuditRecord = {
      id: createId(),
  ...
  async update(tenantId: string, id: string, input: ApprovalsAuditUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: ApprovalsAuditRecord = {
      ...current,
      ...input,
  ...
  async save(record: ApprovalsAuditRecord) {
    records.set(record.id, record);
    return record;
  },
};
