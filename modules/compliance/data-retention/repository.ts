...
import type { DataRetentionCreateInput, DataRetentionListRequest, DataRetentionRecord, DataRetentionUpdateInput } from "./types";

const records = new Map<string, DataRetentionRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const dataRetentionRepository = {
  async list(request: DataRetentionListRequest): Promise<PageResult<DataRetentionRecord>> {
  ...
  async create(input: DataRetentionCreateInput, actorId = "system"): Promise<DataRetentionRecord> {
    const timestamp = now();
  ...
  async update(tenantId: string, id: string, input: DataRetentionUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: DataRetentionRecord = {
      ...current,
      ...input,
  ...
  async save(record: DataRetentionRecord) {
    records.set(record.id, record);
    return record;
  },
};
