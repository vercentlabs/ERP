...
import type { CertificationsCreateInput, CertificationsListRequest, CertificationsRecord, CertificationsUpdateInput } from "./types";

const records = new Map<string, CertificationsRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const certificationsRepository = {
  async list(request: CertificationsListRequest): Promise<PageResult<CertificationsRecord>> {
  ...
  async create(input: CertificationsCreateInput, actorId = "system"): Promise<CertificationsRecord> {
    const timestamp = now();
    const record: CertificationsRecord = {
      id: createId(),
  ...
  async update(tenantId: string, id: string, input: CertificationsUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: CertificationsRecord = {
      ...current,
      ...input,
  ...
  async save(record: CertificationsRecord) {
    records.set(record.id, record);
    return record;
  },
};
