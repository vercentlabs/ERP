...
import type { TaxComplianceCreateInput, TaxComplianceListRequest, TaxComplianceRecord, TaxComplianceUpdateInput } from "./types";

const records = new Map<string, TaxComplianceRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const taxComplianceRepository = {
  async list(request: TaxComplianceListRequest): Promise<PageResult<TaxComplianceRecord>> {
  ...
  async create(input: TaxComplianceCreateInput, actorId = "system"): Promise<TaxComplianceRecord> {
    const timestamp = now();
  ...
  async update(tenantId: string, id: string, input: TaxComplianceUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: TaxComplianceRecord = {
      ...current,
      ...input,
  ...
  async save(record: TaxComplianceRecord) {
    records.set(record.id, record);
    return record;
  },
};
