...
import type { ControlsCreateInput, ControlsListRequest, ControlsRecord, ControlsUpdateInput } from "./types";

const records = new Map<string, ControlsRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const controlsRepository = {
  async list(request: ControlsListRequest): Promise<PageResult<ControlsRecord>> {
  ...
  async create(input: ControlsCreateInput, actorId = "system"): Promise<ControlsRecord> {
    const timestamp = now();
  ...
  async update(tenantId: string, id: string, input: ControlsUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: ControlsRecord = {
      ...current,
      ...input,
  ...
  async save(record: ControlsRecord) {
    records.set(record.id, record);
    return record;
  },
};
