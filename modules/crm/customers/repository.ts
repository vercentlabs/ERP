...
import type { CustomersCreateInput, CustomersListRequest, CustomersRecord, CustomersUpdateInput } from "./types";

const records = new Map<string, CustomersRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const customersRepository = {
  async list(request: CustomersListRequest): Promise<PageResult<CustomersRecord>> {
  ...
  async create(input: CustomersCreateInput, actorId = "system"): Promise<CustomersRecord> {
    const timestamp = now();
  ...
  async update(tenantId: string, id: string, input: CustomersUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: CustomersRecord = {
      ...current,
      ...input,
  ...
  async save(record: CustomersRecord) {
    records.set(record.id, record);
    return record;
  },
};
