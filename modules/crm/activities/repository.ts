...
import type { ActivitiesCreateInput, ActivitiesListRequest, ActivitiesRecord, ActivitiesUpdateInput } from "./types";

const records = new Map<string, ActivitiesRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const activitiesRepository = {
  async list(request: ActivitiesListRequest): Promise<PageResult<ActivitiesRecord>> {
  ...
  async create(input: ActivitiesCreateInput, actorId = "system"): Promise<ActivitiesRecord> {
    const timestamp = now();
    const record: ActivitiesRecord = {
      id: createId(),
  ...
  async update(tenantId: string, id: string, input: ActivitiesUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: ActivitiesRecord = {
      ...current,
      ...input,
  ...
  async save(record: ActivitiesRecord) {
    records.set(record.id, record);
    return record;
  },
};
