...
import type { AccountsCreateInput, AccountsListRequest, AccountsRecord, AccountsUpdateInput } from "./types";

const records = new Map<string, AccountsRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const accountsRepository = {
  async list(request: AccountsListRequest): Promise<PageResult<AccountsRecord>> {
  ...
  async create(input: AccountsCreateInput, actorId = "system"): Promise<AccountsRecord> {
    const timestamp = now();
  ...
  async update(tenantId: string, id: string, input: AccountsUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: AccountsRecord = {
      ...current,
      ...input,
  ...
  async save(record: AccountsRecord) {
    records.set(record.id, record);
    return record;
  },
};
