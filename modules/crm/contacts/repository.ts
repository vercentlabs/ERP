...
import type { ContactsCreateInput, ContactsListRequest, ContactsRecord, ContactsUpdateInput } from "./types";

const records = new Map<string, ContactsRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const contactsRepository = {
  async list(request: ContactsListRequest): Promise<PageResult<ContactsRecord>> {
  ...
  async create(input: ContactsCreateInput, actorId = "system"): Promise<ContactsRecord> {
    const timestamp = now();
    const record: ContactsRecord = {
      id: createId(),
  ...
  async update(tenantId: string, id: string, input: ContactsUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: ContactsRecord = {
      ...current,
      ...input,
  ...
  async save(record: ContactsRecord) {
    records.set(record.id, record);
    return record;
  },
};
