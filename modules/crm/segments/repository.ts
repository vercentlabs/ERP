...
import type { SegmentsCreateInput, SegmentsListRequest, SegmentsRecord, SegmentsUpdateInput } from "./types";

const records = new Map<string, SegmentsRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const segmentsRepository = {
  async list(request: SegmentsListRequest): Promise<PageResult<SegmentsRecord>> {
  ...
  async create(input: SegmentsCreateInput, actorId = "system"): Promise<SegmentsRecord> {
    const timestamp = now();
    const record: SegmentsRecord = {
      id: createId(),
  ...
  async update(tenantId: string, id: string, input: SegmentsUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: SegmentsRecord = {
      ...current,
      ...input,
  ...
  async save(record: SegmentsRecord) {
    records.set(record.id, record);
    return record;
  },
};
