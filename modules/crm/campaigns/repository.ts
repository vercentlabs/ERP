...
import type { CampaignsCreateInput, CampaignsListRequest, CampaignsRecord, CampaignsUpdateInput } from "./types";

const records = new Map<string, CampaignsRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const campaignsRepository = {
  async list(request: CampaignsListRequest): Promise<PageResult<CampaignsRecord>> {
  ...
  async create(input: CampaignsCreateInput, actorId = "system"): Promise<CampaignsRecord> {
    const timestamp = now();
    const record: CampaignsRecord = {
      id: createId(),
  ...
  async update(tenantId: string, id: string, input: CampaignsUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: CampaignsRecord = {
      ...current,
      ...input,
  ...
  async save(record: CampaignsRecord) {
    records.set(record.id, record);
    return record;
  },
};
