...
import type { LeadScoringCreateInput, LeadScoringListRequest, LeadScoringRecord, LeadScoringUpdateInput } from "./types";

const records = new Map<string, LeadScoringRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const leadScoringRepository = {
  async list(request: LeadScoringListRequest): Promise<PageResult<LeadScoringRecord>> {
  ...
  async create(input: LeadScoringCreateInput, actorId = "system"): Promise<LeadScoringRecord> {
    const timestamp = now();
    const record: LeadScoringRecord = {
      id: createId(),
  ...
  async update(tenantId: string, id: string, input: LeadScoringUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: LeadScoringRecord = {
      ...current,
      ...input,
  ...
  async save(record: LeadScoringRecord) {
    records.set(record.id, record);
    return record;
  },
};
