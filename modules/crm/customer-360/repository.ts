...
import type { Customer360CreateInput, Customer360ListRequest, Customer360Record, Customer360UpdateInput } from "./types";

const records = new Map<string, Customer360Record>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const customer360Repository = {
  async list(request: Customer360ListRequest): Promise<PageResult<Customer360Record>> {
  ...
  async create(input: Customer360CreateInput, actorId = "system"): Promise<Customer360Record> {
    const timestamp = now();
    const record: Customer360Record = {
      id: createId(),
  ...
  async update(tenantId: string, id: string, input: Customer360UpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: Customer360Record = {
      ...current,
      ...input,
  ...
  async save(record: Customer360Record) {
    records.set(record.id, record);
    return record;
  },
};
