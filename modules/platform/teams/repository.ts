...
import type { TeamsCreateInput, TeamsListRequest, TeamsRecord, TeamsUpdateInput } from "./types";

const records = new Map<string, TeamsRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const teamsRepository = {
  async list(request: TeamsListRequest): Promise<PageResult<TeamsRecord>> {
  ...
  async create(input: TeamsCreateInput, actorId = "system"): Promise<TeamsRecord> {
    const timestamp = now();
  ...
  async update(tenantId: string, id: string, input: TeamsUpdateInput, actorId = "system") {
