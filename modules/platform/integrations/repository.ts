...
import type { IntegrationsCreateInput, IntegrationsListRequest, IntegrationsRecord, IntegrationsUpdateInput } from "./types";

const records = new Map<string, IntegrationsRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const integrationsRepository = {
  async list(request: IntegrationsListRequest): Promise<PageResult<IntegrationsRecord>> {
  ...
  async create(input: IntegrationsCreateInput, actorId = "system"): Promise<IntegrationsRecord> {
    const timestamp = now();
  ...
  async update(tenantId: string, id: string, input: IntegrationsUpdateInput, actorId = "system") {
  ...
  async save(record: IntegrationsRecord) {
    records.set(record.id, record);
    return record;
  },
};
