...
import type { DocumentsCreateInput, DocumentsListRequest, DocumentsRecord, DocumentsUpdateInput } from "./types";

const records = new Map<string, DocumentsRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const documentsRepository = {
  async list(request: DocumentsListRequest): Promise<PageResult<DocumentsRecord>> {
  ...
  async create(input: DocumentsCreateInput, actorId = "system"): Promise<DocumentsRecord> {
    const timestamp = now();
  ...
  async update(tenantId: string, id: string, input: DocumentsUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: DocumentsRecord = {
      ...current,
      ...input,
  ...
  async save(record: DocumentsRecord) {
    records.set(record.id, record);
    return record;
  },
};
