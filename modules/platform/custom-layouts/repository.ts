...
import type { CustomLayoutsCreateInput, CustomLayoutsListRequest, CustomLayoutsRecord, CustomLayoutsUpdateInput } from "./types";

const records = new Map<string, CustomLayoutsRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const customLayoutsRepository = {
  async list(request: CustomLayoutsListRequest): Promise<PageResult<CustomLayoutsRecord>> {
  ...
  async create(input: CustomLayoutsCreateInput, actorId = "system"): Promise<CustomLayoutsRecord> {
    const timestamp = now();
    const record: CustomLayoutsRecord = {
      id: createId(),
  ...
  async update(tenantId: string, id: string, input: CustomLayoutsUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: CustomLayoutsRecord = {
      ...current,
      ...input,
  ...
  async save(record: CustomLayoutsRecord) {
    records.set(record.id, record);
    return record;
  },
};
