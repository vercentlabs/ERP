...
import type { LocalizationCreateInput, LocalizationListRequest, LocalizationRecord, LocalizationUpdateInput } from "./types";
...
export const localizationRepository = {
  async list(request: LocalizationListRequest): Promise<PageResult<LocalizationRecord>> {
  ...
  async create(input: LocalizationCreateInput, actorId = "system"): Promise<LocalizationRecord> {
    const timestamp = now();
  ...
  async update(tenantId: string, id: string, input: LocalizationUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: LocalizationRecord = {
      ...current,
      ...input,
  ...
  async save(record: LocalizationRecord) {
    records.set(record.id, record);
    return record;
  },
};
