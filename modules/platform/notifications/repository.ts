...
import type { NotificationsCreateInput, NotificationsListRequest, NotificationsRecord, NotificationsUpdateInput } from "./types";

const records = new Map<string, NotificationsRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const notificationsRepository = {
  async list(request: NotificationsListRequest): Promise<PageResult<NotificationsRecord>> {
  ...
  async create(input: NotificationsCreateInput, actorId = "system"): Promise<NotificationsRecord> {
    const timestamp = now();
  ...
  async update(tenantId: string, id: string, input: NotificationsUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: NotificationsRecord = {
      ...current,
      ...input,
  ...
  async save(record: NotificationsRecord) {
    records.set(record.id, record);
    return record;
  },
};
