...
import type { BranchesCreateInput, BranchesListRequest, BranchesRecord, BranchesUpdateInput } from "./types";

const records = new Map<string, BranchesRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const branchesRepository = {
  async list(request: BranchesListRequest): Promise<PageResult<BranchesRecord>> {
  ...
  async create(input: BranchesCreateInput, actorId = "system"): Promise<BranchesRecord> {
    const timestamp = now();
    const record: BranchesRecord = {
      id: createId(),
  ...
  async update(tenantId: string, id: string, input: BranchesUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: BranchesRecord = {
      ...current,
      ...input,
  ...
  async save(record: BranchesRecord) {
    records.set(record.id, record);
    return record;
  },
};
