...
import type { SettingsCreateInput, SettingsListRequest, SettingsRecord, SettingsUpdateInput } from "./types";
...
export const settingsRepository = {
  async list(request: SettingsListRequest): Promise<PageResult<SettingsRecord>> {
  ...
  async create(input: SettingsCreateInput, actorId = "system"): Promise<SettingsRecord> {
    const timestamp = now();
  ...
  async update(tenantId: string, id: string, input: SettingsUpdateInput, actorId = "system") {
  ...
  async save(record: SettingsRecord) {
    records.set(record.id, record);
    return record;
  },
};
