...
import type { CompaniesCreateInput, CompaniesListRequest, CompaniesRecord, CompaniesUpdateInput } from "./types";

const records = new Map<string, CompaniesRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const companiesRepository = {
  async list(request: CompaniesListRequest): Promise<PageResult<CompaniesRecord>> {
  ...
  async create(input: CompaniesCreateInput, actorId = "system"): Promise<CompaniesRecord> {
    const timestamp = now();
    const record: CompaniesRecord = {
      id: createId(),
  ...
  async update(tenantId: string, id: string, input: CompaniesUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: CompaniesRecord = {
      ...current,
      ...input,
  ...
  async save(record: CompaniesRecord) {
    records.set(record.id, record);
    return record;
  },
};
