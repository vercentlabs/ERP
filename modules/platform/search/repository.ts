...
import type { SearchCreateInput, SearchListRequest, SearchRecord, SearchUpdateInput } from "./types";
...
export const searchRepository = {
  async list(request: SearchListRequest): Promise<PageResult<SearchRecord>> {
  ...
  async create(input: SearchCreateInput, actorId = "system"): Promise<SearchRecord> {
    const timestamp = now();
  ...
  async update(tenantId: string, id: string, input: SearchUpdateInput, actorId = "system") {
  ...
  async save(record: SearchRecord) {
    records.set(record.id, record);
    return record;
  },
};
