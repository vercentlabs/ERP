...
import type { AuditLogsCreateInput, AuditLogsListRequest, AuditLogsRecord, AuditLogsUpdateInput } from "./types";

const records = new Map<string, AuditLogsRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const auditLogsRepository = {
  async list(request: AuditLogsListRequest): Promise<PageResult<AuditLogsRecord>> {
  ...
  async create(input: AuditLogsCreateInput, actorId = "system"): Promise<AuditLogsRecord> {
    const timestamp = now();
    const record: AuditLogsRecord = {
      id: createId(),
  ...
  async update(tenantId: string, id: string, input: AuditLogsUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: AuditLogsRecord = {
      ...current,
      ...input,
  ...
  async save(record: AuditLogsRecord) {
    records.set(record.id, record);
    return record;
  },
};
