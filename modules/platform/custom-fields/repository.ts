...
import type { CustomFieldsCreateInput, CustomFieldsListRequest, CustomFieldsRecord, CustomFieldsUpdateInput } from "./types";

const records = new Map<string, CustomFieldsRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const customFieldsRepository = {
  async list(request: CustomFieldsListRequest): Promise<PageResult<CustomFieldsRecord>> {
  ...
  async create(input: CustomFieldsCreateInput, actorId = "system"): Promise<CustomFieldsRecord> {
    const timestamp = now();
    const record: CustomFieldsRecord = {
      id: createId(),
  ...
  async update(tenantId: string, id: string, input: CustomFieldsUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: CustomFieldsRecord = {
      ...current,
      ...input,
  ...
  async save(record: CustomFieldsRecord) {
    records.set(record.id, record);
    return record;
  },
};
