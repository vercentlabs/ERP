...
import type { CommentsCreateInput, CommentsListRequest, CommentsRecord, CommentsUpdateInput } from "./types";

const records = new Map<string, CommentsRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const commentsRepository = {
  async list(request: CommentsListRequest): Promise<PageResult<CommentsRecord>> {
  ...
  async create(input: CommentsCreateInput, actorId = "system"): Promise<CommentsRecord> {
    const timestamp = now();
    const record: CommentsRecord = {
      id: createId(),
  ...
  async update(tenantId: string, id: string, input: CommentsUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: CommentsRecord = {
      ...current,
      ...input,
  ...
  async save(record: CommentsRecord) {
    records.set(record.id, record);
    return record;
  },
};
