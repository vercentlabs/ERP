...
import type { ImportsCreateInput, ImportsListRequest, ImportsRecord, ImportsUpdateInput } from "./types";

const records = new Map<string, ImportsRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const importsRepository = {
  async list(request: ImportsListRequest): Promise<PageResult<ImportsRecord>> {
  ...
  async create(input: ImportsCreateInput, actorId = "system"): Promise<ImportsRecord> {
    const timestamp = now();
    const record: ImportsRecord = {
      id: createId(),
  ...
  async update(tenantId: string, id: string, input: ImportsUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: ImportsRecord = {
      ...current,
      ...input,
  ...
  async save(record: ImportsRecord) {
    records.set(record.id, record);
    return record;
  },
};
