...
import type { UsersCreateInput, UsersListRequest, UsersRecord, UsersUpdateInput } from "./types";

const records = new Map<string, UsersRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const usersRepository = {
  async list(request: UsersListRequest): Promise<PageResult<UsersRecord>> {
  ...
  async create(input: UsersCreateInput, actorId = "system"): Promise<UsersRecord> {
    const timestamp = now();
    const record: UsersRecord = {
      id: createId(),
  ...
  async update(tenantId: string, id: string, input: UsersUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: UsersRecord = {
      ...current,
      ...input,
