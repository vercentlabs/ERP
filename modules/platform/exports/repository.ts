...
import type { ExportsCreateInput, ExportsListRequest, ExportsRecord, ExportsUpdateInput } from "./types";

const records = new Map<string, ExportsRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const exportsRepository = {
  async list(request: ExportsListRequest): Promise<PageResult<ExportsRecord>> {
  ...
  async create(input: ExportsCreateInput, actorId = "system"): Promise<ExportsRecord> {
    const timestamp = now();
    const record: ExportsRecord = {
      id: createId(),
  ...
  async update(tenantId: string, id: string, input: ExportsUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: ExportsRecord = {
      ...current,
      ...input,
  ...
  async save(record: ExportsRecord) {
    records.set(record.id, record);
    return record;
  },
};
