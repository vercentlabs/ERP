...
import type { TaxMasterCreateInput, TaxMasterListRequest, TaxMasterRecord, TaxMasterUpdateInput } from "./types";

const records = new Map<string, TaxMasterRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const taxMasterRepository = {
  async list(request: TaxMasterListRequest): Promise<PageResult<TaxMasterRecord>> {
  ...
  async create(input: TaxMasterCreateInput, actorId = "system"): Promise<TaxMasterRecord> {
    const timestamp = now();
  ...
  async update(tenantId: string, id: string, input: TaxMasterUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: TaxMasterRecord = {
      ...current,
      ...input,
  ...
  async save(record: TaxMasterRecord) {
    records.set(record.id, record);
    return record;
  },
};
