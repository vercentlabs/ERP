...
import type { ChartMasterCreateInput, ChartMasterListRequest, ChartMasterRecord, ChartMasterUpdateInput } from "./types";

const records = new Map<string, ChartMasterRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const chartMasterRepository = {
  async list(request: ChartMasterListRequest): Promise<PageResult<ChartMasterRecord>> {
  ...
  async create(input: ChartMasterCreateInput, actorId = "system"): Promise<ChartMasterRecord> {
    const timestamp = now();
    const record: ChartMasterRecord = {
      id: createId(),
  ...
  async update(tenantId: string, id: string, input: ChartMasterUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: ChartMasterRecord = {
      ...current,
      ...input,
  ...
  async save(record: ChartMasterRecord) {
    records.set(record.id, record);
    return record;
  },
};
