...
import type { PartiesCreateInput, PartiesListRequest, PartiesRecord, PartiesUpdateInput } from "./types";

const records = new Map<string, PartiesRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const partiesRepository = {
  async list(request: PartiesListRequest): Promise<PageResult<PartiesRecord>> {
  ...
  async create(input: PartiesCreateInput, actorId = "system"): Promise<PartiesRecord> {
    const timestamp = now();
    const record: PartiesRecord = {
      id: createId(),
  ...
  async update(tenantId: string, id: string, input: PartiesUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: PartiesRecord = {
      ...current,
      ...input,
  ...
  async save(record: PartiesRecord) {
    records.set(record.id, record);
    return record;
  },
};
