...
import type { CurrencyMasterCreateInput, CurrencyMasterListRequest, CurrencyMasterRecord, CurrencyMasterUpdateInput } from "./types";

const records = new Map<string, CurrencyMasterRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const currencyMasterRepository = {
  async list(request: CurrencyMasterListRequest): Promise<PageResult<CurrencyMasterRecord>> {
  ...
  async create(input: CurrencyMasterCreateInput, actorId = "system"): Promise<CurrencyMasterRecord> {
    const timestamp = now();
    const record: CurrencyMasterRecord = {
      id: createId(),
  ...
  async update(tenantId: string, id: string, input: CurrencyMasterUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: CurrencyMasterRecord = {
      ...current,
      ...input,
  ...
  async save(record: CurrencyMasterRecord) {
    records.set(record.id, record);
    return record;
  },
};
