...
import type { DataGovernanceCreateInput, DataGovernanceListRequest, DataGovernanceRecord, DataGovernanceUpdateInput } from "./types";

const records = new Map<string, DataGovernanceRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const dataGovernanceRepository = {
  async list(request: DataGovernanceListRequest): Promise<PageResult<DataGovernanceRecord>> {
  ...
  async create(input: DataGovernanceCreateInput, actorId = "system"): Promise<DataGovernanceRecord> {
    const timestamp = now();
    const record: DataGovernanceRecord = {
      id: createId(),
  ...
  async update(tenantId: string, id: string, input: DataGovernanceUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: DataGovernanceRecord = {
      ...current,
      ...input,
  ...
  async save(record: DataGovernanceRecord) {
    records.set(record.id, record);
    return record;
  },
};
