...
import type { UomMasterCreateInput, UomMasterListRequest, UomMasterRecord, UomMasterUpdateInput } from "./types";

const records = new Map<string, UomMasterRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const uomMasterRepository = {
  async list(request: UomMasterListRequest): Promise<PageResult<UomMasterRecord>> {
  ...
  async create(input: UomMasterCreateInput, actorId = "system"): Promise<UomMasterRecord> {
    const timestamp = now();
    const record: UomMasterRecord = {
      id: createId(),
  ...
  async update(tenantId: string, id: string, input: UomMasterUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: UomMasterRecord = {
      ...current,
      ...input,
  ...
  async save(record: UomMasterRecord) {
    records.set(record.id, record);
    return record;
  },
};
