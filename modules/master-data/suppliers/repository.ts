...
import type { SuppliersCreateInput, SuppliersListRequest, SuppliersRecord, SuppliersUpdateInput } from "./types";

const records = new Map<string, SuppliersRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const suppliersRepository = {
  async list(request: SuppliersListRequest): Promise<PageResult<SuppliersRecord>> {
  ...
  async create(input: SuppliersCreateInput, actorId = "system"): Promise<SuppliersRecord> {
    const timestamp = now();
  ...
  async update(tenantId: string, id: string, input: SuppliersUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: SuppliersRecord = {
      ...current,
      ...input,
  ...
  async save(record: SuppliersRecord) {
    records.set(record.id, record);
    return record;
  },
};
