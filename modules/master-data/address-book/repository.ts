...
import type { AddressBookCreateInput, AddressBookListRequest, AddressBookRecord, AddressBookUpdateInput } from "./types";

const records = new Map<string, AddressBookRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const addressBookRepository = {
  async list(request: AddressBookListRequest): Promise<PageResult<AddressBookRecord>> {
  ...
  async create(input: AddressBookCreateInput, actorId = "system"): Promise<AddressBookRecord> {
    const timestamp = now();
  ...
  async update(tenantId: string, id: string, input: AddressBookUpdateInput, actorId = "system") {
  ...
  async save(record: AddressBookRecord) {
    records.set(record.id, record);
    return record;
  },
};
