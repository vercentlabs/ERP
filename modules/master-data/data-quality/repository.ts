...
import type { DataQualityCreateInput, DataQualityListRequest, DataQualityRecord, DataQualityUpdateInput } from "./types";

const records = new Map<string, DataQualityRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const dataQualityRepository = {
  async list(request: DataQualityListRequest): Promise<PageResult<DataQualityRecord>> {
  ...
  async create(input: DataQualityCreateInput, actorId = "system"): Promise<DataQualityRecord> {
    const timestamp = now();
  ...
  async update(tenantId: string, id: string, input: DataQualityUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: DataQualityRecord = {
      ...current,
      ...input,
  ...
  async save(record: DataQualityRecord) {
    records.set(record.id, record);
    return record;
  },
};
