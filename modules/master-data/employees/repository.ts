...
import type { EmployeesCreateInput, EmployeesListRequest, EmployeesRecord, EmployeesUpdateInput } from "./types";

const records = new Map<string, EmployeesRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const employeesRepository = {
  async list(request: EmployeesListRequest): Promise<PageResult<EmployeesRecord>> {
  ...
  async create(input: EmployeesCreateInput, actorId = "system"): Promise<EmployeesRecord> {
    const timestamp = now();
    const record: EmployeesRecord = {
      id: createId(),
  ...
  async update(tenantId: string, id: string, input: EmployeesUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: EmployeesRecord = {
      ...current,
      ...input,
  ...
  async save(record: EmployeesRecord) {
    records.set(record.id, record);
    return record;
  },
};
