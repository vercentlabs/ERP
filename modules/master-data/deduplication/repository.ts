...
import type { DeduplicationCreateInput, DeduplicationListRequest, DeduplicationRecord, DeduplicationUpdateInput } from "./types";

const records = new Map<string, DeduplicationRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const deduplicationRepository = {
  async list(request: DeduplicationListRequest): Promise<PageResult<DeduplicationRecord>> {
  ...
  async create(input: DeduplicationCreateInput, actorId = "system"): Promise<DeduplicationRecord> {
    const timestamp = now();
  ...
  async update(tenantId: string, id: string, input: DeduplicationUpdateInput, actorId = "system") {
  ...
  async save(record: DeduplicationRecord) {
    records.set(record.id, record);
    return record;
  },
};
