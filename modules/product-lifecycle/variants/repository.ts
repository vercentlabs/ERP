...
import type { VariantsCreateInput, VariantsListRequest, VariantsRecord, VariantsUpdateInput } from "./types";

const records = new Map<string, VariantsRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const variantsRepository = {
  async list(request: VariantsListRequest): Promise<PageResult<VariantsRecord>> {
  ...
  async create(input: VariantsCreateInput, actorId = "system"): Promise<VariantsRecord> {
    const timestamp = now();
    const record: VariantsRecord = {
      id: createId(),
  ...
  async update(tenantId: string, id: string, input: VariantsUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: VariantsRecord = {
      ...current,
      ...input,
  ...
  async save(record: VariantsRecord) {
    records.set(record.id, record);
    return record;
  },
};
