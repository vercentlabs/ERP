...
import type { PurchaseReturnsCreateInput, PurchaseReturnsListRequest, PurchaseReturnsRecord, PurchaseReturnsUpdateInput } from "./types";

const records = new Map<string, PurchaseReturnsRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const purchaseReturnsRepository = {
  async list(request: PurchaseReturnsListRequest): Promise<PageResult<PurchaseReturnsRecord>> {
  ...
  async create(input: PurchaseReturnsCreateInput, actorId = "system"): Promise<PurchaseReturnsRecord> {
    const timestamp = now();
    const record: PurchaseReturnsRecord = {
      id: createId(),
  ...
  async update(tenantId: string, id: string, input: PurchaseReturnsUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: PurchaseReturnsRecord = {
      ...current,
      ...input,
  ...
  async save(record: PurchaseReturnsRecord) {
    records.set(record.id, record);
    return record;
  },
};
