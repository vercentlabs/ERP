...
import type { RfqsCreateInput, RfqsListRequest, RfqsRecord, RfqsUpdateInput } from "./types";

const records = new Map<string, RfqsRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const rfqsRepository = {
  async list(request: RfqsListRequest): Promise<PageResult<RfqsRecord>> {
  ...
  async create(input: RfqsCreateInput, actorId = "system"): Promise<RfqsRecord> {
    const timestamp = now();
    const record: RfqsRecord = {
      id: createId(),
  ...
  async update(tenantId: string, id: string, input: RfqsUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: RfqsRecord = {
      ...current,
      ...input,
