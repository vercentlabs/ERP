...
import type { GoodsReceiptsCreateInput, GoodsReceiptsListRequest, GoodsReceiptsRecord, GoodsReceiptsUpdateInput } from "./types";

const records = new Map<string, GoodsReceiptsRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const goodsReceiptsRepository = {
  async list(request: GoodsReceiptsListRequest): Promise<PageResult<GoodsReceiptsRecord>> {
  ...
  async create(input: GoodsReceiptsCreateInput, actorId = "system"): Promise<GoodsReceiptsRecord> {
    const timestamp = now();
    const record: GoodsReceiptsRecord = {
      id: createId(),
  ...
  async update(tenantId: string, id: string, input: GoodsReceiptsUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: GoodsReceiptsRecord = {
      ...current,
      ...input,
  ...
  async save(record: GoodsReceiptsRecord) {
    records.set(record.id, record);
    return record;
  },
};
