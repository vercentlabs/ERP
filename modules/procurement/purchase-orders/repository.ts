...
import type { PurchaseOrdersCreateInput, PurchaseOrdersListRequest, PurchaseOrdersRecord, PurchaseOrdersUpdateInput } from "./types";

const records = new Map<string, PurchaseOrdersRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const purchaseOrdersRepository = {
  async list(request: PurchaseOrdersListRequest): Promise<PageResult<PurchaseOrdersRecord>> {
  ...
  async create(input: PurchaseOrdersCreateInput, actorId = "system"): Promise<PurchaseOrdersRecord> {
    const timestamp = now();
    const record: PurchaseOrdersRecord = {
      id: createId(),
  ...
  async update(tenantId: string, id: string, input: PurchaseOrdersUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: PurchaseOrdersRecord = {
      ...current,
      ...input,
  ...
  async save(record: PurchaseOrdersRecord) {
    records.set(record.id, record);
    return record;
  },
};
