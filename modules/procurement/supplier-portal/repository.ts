...
import type { SupplierPortalCreateInput, SupplierPortalListRequest, SupplierPortalRecord, SupplierPortalUpdateInput } from "./types";

const records = new Map<string, SupplierPortalRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const supplierPortalRepository = {
  async list(request: SupplierPortalListRequest): Promise<PageResult<SupplierPortalRecord>> {
  ...
  async create(input: SupplierPortalCreateInput, actorId = "system"): Promise<SupplierPortalRecord> {
    const timestamp = now();
    const record: SupplierPortalRecord = {
      id: createId(),
  ...
  async update(tenantId: string, id: string, input: SupplierPortalUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: SupplierPortalRecord = {
      ...current,
      ...input,
  ...
  async save(record: SupplierPortalRecord) {
    records.set(record.id, record);
    return record;
  },
};
