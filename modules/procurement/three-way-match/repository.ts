...
import type { ThreeWayMatchCreateInput, ThreeWayMatchListRequest, ThreeWayMatchRecord, ThreeWayMatchUpdateInput } from "./types";

const records = new Map<string, ThreeWayMatchRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const threeWayMatchRepository = {
  async list(request: ThreeWayMatchListRequest): Promise<PageResult<ThreeWayMatchRecord>> {
  ...
  async create(input: ThreeWayMatchCreateInput, actorId = "system"): Promise<ThreeWayMatchRecord> {
    const timestamp = now();
  ...
  async update(tenantId: string, id: string, input: ThreeWayMatchUpdateInput, actorId = "system") {
  ...
  async save(record: ThreeWayMatchRecord) {
    records.set(record.id, record);
    return record;
  },
};
