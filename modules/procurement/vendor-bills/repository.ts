...
import type { VendorBillsCreateInput, VendorBillsListRequest, VendorBillsRecord, VendorBillsUpdateInput } from "./types";

const records = new Map<string, VendorBillsRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const vendorBillsRepository = {
  async list(request: VendorBillsListRequest): Promise<PageResult<VendorBillsRecord>> {
  ...
  async create(input: VendorBillsCreateInput, actorId = "system"): Promise<VendorBillsRecord> {
    const timestamp = now();
    const record: VendorBillsRecord = {
      id: createId(),
  ...
  async update(tenantId: string, id: string, input: VendorBillsUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: VendorBillsRecord = {
      ...current,
      ...input,
  ...
  async save(record: VendorBillsRecord) {
    records.set(record.id, record);
    return record;
  },
};
