...
import type { LandedCostsCreateInput, LandedCostsListRequest, LandedCostsRecord, LandedCostsUpdateInput } from "./types";

const records = new Map<string, LandedCostsRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const landedCostsRepository = {
  async list(request: LandedCostsListRequest): Promise<PageResult<LandedCostsRecord>> {
  ...
  async create(input: LandedCostsCreateInput, actorId = "system"): Promise<LandedCostsRecord> {
    const timestamp = now();
    const record: LandedCostsRecord = {
      id: createId(),
  ...
  async update(tenantId: string, id: string, input: LandedCostsUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: LandedCostsRecord = {
      ...current,
      ...input,
  ...
  async save(record: LandedCostsRecord) {
    records.set(record.id, record);
    return record;
  },
};
