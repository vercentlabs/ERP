...
import type { BlanketOrdersCreateInput, BlanketOrdersListRequest, BlanketOrdersRecord, BlanketOrdersUpdateInput } from "./types";

const records = new Map<string, BlanketOrdersRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const blanketOrdersRepository = {
  async list(request: BlanketOrdersListRequest): Promise<PageResult<BlanketOrdersRecord>> {
  ...
  async create(input: BlanketOrdersCreateInput, actorId = "system"): Promise<BlanketOrdersRecord> {
    const timestamp = now();
    const record: BlanketOrdersRecord = {
      id: createId(),
  ...
  async update(tenantId: string, id: string, input: BlanketOrdersUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: BlanketOrdersRecord = {
      ...current,
      ...input,
  ...
  async save(record: BlanketOrdersRecord) {
    records.set(record.id, record);
    return record;
  },
};
