...
import type { SdkManagementCreateInput, SdkManagementListRequest, SdkManagementRecord, SdkManagementUpdateInput } from "./types";

const records = new Map<string, SdkManagementRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const sdkManagementRepository = {
  async list(request: SdkManagementListRequest): Promise<PageResult<SdkManagementRecord>> {
  ...
  async create(input: SdkManagementCreateInput, actorId = "system"): Promise<SdkManagementRecord> {
    const timestamp = now();
    const record: SdkManagementRecord = {
      id: createId(),
  ...
  async update(tenantId: string, id: string, input: SdkManagementUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: SdkManagementRecord = {
      ...current,
      ...input,
  ...
  async save(record: SdkManagementRecord) {
    records.set(record.id, record);
    return record;
  },
};
