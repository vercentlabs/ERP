...
import type { InstalledAppsCreateInput, InstalledAppsListRequest, InstalledAppsRecord, InstalledAppsUpdateInput } from "./types";

const records = new Map<string, InstalledAppsRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const installedAppsRepository = {
  async list(request: InstalledAppsListRequest): Promise<PageResult<InstalledAppsRecord>> {
  ...
  async create(input: InstalledAppsCreateInput, actorId = "system"): Promise<InstalledAppsRecord> {
    const timestamp = now();
    const record: InstalledAppsRecord = {
      id: createId(),
  ...
  async update(tenantId: string, id: string, input: InstalledAppsUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: InstalledAppsRecord = {
      ...current,
      ...input,
  ...
  async save(record: InstalledAppsRecord) {
    records.set(record.id, record);
    return record;
  },
};
