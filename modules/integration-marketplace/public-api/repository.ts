...
import type { PublicApiCreateInput, PublicApiListRequest, PublicApiRecord, PublicApiUpdateInput } from "./types";

const records = new Map<string, PublicApiRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const publicApiRepository = {
  async list(request: PublicApiListRequest): Promise<PageResult<PublicApiRecord>> {
  ...
  async create(input: PublicApiCreateInput, actorId = "system"): Promise<PublicApiRecord> {
    const timestamp = now();
    const record: PublicApiRecord = {
      id: createId(),
  ...
  async update(tenantId: string, id: string, input: PublicApiUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: PublicApiRecord = {
      ...current,
      ...input,
  ...
  async save(record: PublicApiRecord) {
    records.set(record.id, record);
    return record;
  },
};
