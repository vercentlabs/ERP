...
import type { ConnectorsCreateInput, ConnectorsListRequest, ConnectorsRecord, ConnectorsUpdateInput } from "./types";

const records = new Map<string, ConnectorsRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const connectorsRepository = {
  async list(request: ConnectorsListRequest): Promise<PageResult<ConnectorsRecord>> {
  ...
  async create(input: ConnectorsCreateInput, actorId = "system"): Promise<ConnectorsRecord> {
    const timestamp = now();
    const record: ConnectorsRecord = {
      id: createId(),
  ...
  async update(tenantId: string, id: string, input: ConnectorsUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: ConnectorsRecord = {
      ...current,
      ...input,
  ...
  async save(record: ConnectorsRecord) {
    records.set(record.id, record);
    return record;
  },
};
