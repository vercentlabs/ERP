...
import type { WebhooksCreateInput, WebhooksListRequest, WebhooksRecord, WebhooksUpdateInput } from "./types";

const records = new Map<string, WebhooksRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const webhooksRepository = {
  async list(request: WebhooksListRequest): Promise<PageResult<WebhooksRecord>> {
  ...
  async create(input: WebhooksCreateInput, actorId = "system"): Promise<WebhooksRecord> {
    const timestamp = now();
  ...
  async update(tenantId: string, id: string, input: WebhooksUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: WebhooksRecord = {
      ...current,
      ...input,
  ...
  async save(record: WebhooksRecord) {
    records.set(record.id, record);
    return record;
  },
};
