...
import type { DashboardsCreateInput, DashboardsListRequest, DashboardsRecord, DashboardsUpdateInput } from "./types";

const records = new Map<string, DashboardsRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const dashboardsRepository = {
  async list(request: DashboardsListRequest): Promise<PageResult<DashboardsRecord>> {
  ...
  async create(input: DashboardsCreateInput, actorId = "system"): Promise<DashboardsRecord> {
    const timestamp = now();
    const record: DashboardsRecord = {
      id: createId(),
  ...
  async update(tenantId: string, id: string, input: DashboardsUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: DashboardsRecord = {
      ...current,
      ...input,
  ...
  async save(record: DashboardsRecord) {
    records.set(record.id, record);
    return record;
  },
};
