...
import type { DataMartsCreateInput, DataMartsListRequest, DataMartsRecord, DataMartsUpdateInput } from "./types";

const records = new Map<string, DataMartsRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const dataMartsRepository = {
  async list(request: DataMartsListRequest): Promise<PageResult<DataMartsRecord>> {
  ...
  async create(input: DataMartsCreateInput, actorId = "system"): Promise<DataMartsRecord> {
    const timestamp = now();
    const record: DataMartsRecord = {
      id: createId(),
  ...
  async update(tenantId: string, id: string, input: DataMartsUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: DataMartsRecord = {
      ...current,
      ...input,
  ...
  async save(record: DataMartsRecord) {
    records.set(record.id, record);
    return record;
  },
};
