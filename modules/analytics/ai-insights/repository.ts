...
import type { AiInsightsCreateInput, AiInsightsListRequest, AiInsightsRecord, AiInsightsUpdateInput } from "./types";

const records = new Map<string, AiInsightsRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const aiInsightsRepository = {
  async list(request: AiInsightsListRequest): Promise<PageResult<AiInsightsRecord>> {
  ...
  async create(input: AiInsightsCreateInput, actorId = "system"): Promise<AiInsightsRecord> {
    const timestamp = now();
    const record: AiInsightsRecord = {
      id: createId(),
  ...
  async update(tenantId: string, id: string, input: AiInsightsUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: AiInsightsRecord = {
      ...current,
      ...input,
  ...
  async save(record: AiInsightsRecord) {
    records.set(record.id, record);
    return record;
  },
};
