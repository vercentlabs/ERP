...
import type { KpisCreateInput, KpisListRequest, KpisRecord, KpisUpdateInput } from "./types";

const records = new Map<string, KpisRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const kpisRepository = {
  async list(request: KpisListRequest): Promise<PageResult<KpisRecord>> {
  ...
  async create(input: KpisCreateInput, actorId = "system"): Promise<KpisRecord> {
    const timestamp = now();
    const record: KpisRecord = {
      id: createId(),
  ...
  async update(tenantId: string, id: string, input: KpisUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: KpisRecord = {
      ...current,
      ...input,
