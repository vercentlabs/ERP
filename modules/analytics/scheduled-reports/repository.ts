...
import type { ScheduledReportsCreateInput, ScheduledReportsListRequest, ScheduledReportsRecord, ScheduledReportsUpdateInput } from "./types";

const records = new Map<string, ScheduledReportsRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const scheduledReportsRepository = {
  async list(request: ScheduledReportsListRequest): Promise<PageResult<ScheduledReportsRecord>> {
  ...
  async create(input: ScheduledReportsCreateInput, actorId = "system"): Promise<ScheduledReportsRecord> {
    const timestamp = now();
  ...
  async update(tenantId: string, id: string, input: ScheduledReportsUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: ScheduledReportsRecord = {
      ...current,
      ...input,
  ...
  async save(record: ScheduledReportsRecord) {
    records.set(record.id, record);
    return record;
  },
};
