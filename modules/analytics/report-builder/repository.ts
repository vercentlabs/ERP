...
import type { ReportBuilderCreateInput, ReportBuilderListRequest, ReportBuilderRecord, ReportBuilderUpdateInput } from "./types";

const records = new Map<string, ReportBuilderRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const reportBuilderRepository = {
  async list(request: ReportBuilderListRequest): Promise<PageResult<ReportBuilderRecord>> {
  ...
  async create(input: ReportBuilderCreateInput, actorId = "system"): Promise<ReportBuilderRecord> {
    const timestamp = now();
  ...
  async update(tenantId: string, id: string, input: ReportBuilderUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: ReportBuilderRecord = {
      ...current,
      ...input,
  ...
  async save(record: ReportBuilderRecord) {
    records.set(record.id, record);
    return record;
  },
};
