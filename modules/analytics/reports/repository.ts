...
import type { ReportsCreateInput, ReportsListRequest, ReportsRecord, ReportsUpdateInput } from "./types";

const records = new Map<string, ReportsRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const reportsRepository = {
  async list(request: ReportsListRequest): Promise<PageResult<ReportsRecord>> {
  ...
  async create(input: ReportsCreateInput, actorId = "system"): Promise<ReportsRecord> {
    const timestamp = now();
  ...
  async update(tenantId: string, id: string, input: ReportsUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: ReportsRecord = {
      ...current,
      ...input,
  ...
  async save(record: ReportsRecord) {
    records.set(record.id, record);
    return record;
  },
};
