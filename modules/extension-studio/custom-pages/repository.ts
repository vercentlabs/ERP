...
import type { CustomPagesCreateInput, CustomPagesListRequest, CustomPagesRecord, CustomPagesUpdateInput } from "./types";

const records = new Map<string, CustomPagesRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const customPagesRepository = {
  async list(request: CustomPagesListRequest): Promise<PageResult<CustomPagesRecord>> {
  ...
  async create(input: CustomPagesCreateInput, actorId = "system"): Promise<CustomPagesRecord> {
    const timestamp = now();
    const record: CustomPagesRecord = {
      id: createId(),
  ...
  async update(tenantId: string, id: string, input: CustomPagesUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: CustomPagesRecord = {
      ...current,
      ...input,
  ...
  async save(record: CustomPagesRecord) {
    records.set(record.id, record);
    return record;
  },
};
