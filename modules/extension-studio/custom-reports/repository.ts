...
import type { CustomReportsCreateInput, CustomReportsListRequest, CustomReportsRecord, CustomReportsUpdateInput } from "./types";

const records = new Map<string, CustomReportsRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const customReportsRepository = {
  async list(request: CustomReportsListRequest): Promise<PageResult<CustomReportsRecord>> {
  ...
  async create(input: CustomReportsCreateInput, actorId = "system"): Promise<CustomReportsRecord> {
    const timestamp = now();
    const record: CustomReportsRecord = {
      id: createId(),
  ...
  async update(tenantId: string, id: string, input: CustomReportsUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: CustomReportsRecord = {
      ...current,
      ...input,
  ...
  async save(record: CustomReportsRecord) {
    records.set(record.id, record);
    return record;
  },
};
