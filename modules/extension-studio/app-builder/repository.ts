...
import type { AppBuilderCreateInput, AppBuilderListRequest, AppBuilderRecord, AppBuilderUpdateInput } from "./types";

const records = new Map<string, AppBuilderRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const appBuilderRepository = {
  async list(request: AppBuilderListRequest): Promise<PageResult<AppBuilderRecord>> {
  ...
  async create(input: AppBuilderCreateInput, actorId = "system"): Promise<AppBuilderRecord> {
    const timestamp = now();
    const record: AppBuilderRecord = {
      id: createId(),
  ...
  async update(tenantId: string, id: string, input: AppBuilderUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: AppBuilderRecord = {
      ...current,
      ...input,
  ...
  async save(record: AppBuilderRecord) {
    records.set(record.id, record);
    return record;
  },
};
