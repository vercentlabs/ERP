...
import type { CustomObjectsCreateInput, CustomObjectsListRequest, CustomObjectsRecord, CustomObjectsUpdateInput } from "./types";

const records = new Map<string, CustomObjectsRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const customObjectsRepository = {
  async list(request: CustomObjectsListRequest): Promise<PageResult<CustomObjectsRecord>> {
  ...
  async create(input: CustomObjectsCreateInput, actorId = "system"): Promise<CustomObjectsRecord> {
    const timestamp = now();
    const record: CustomObjectsRecord = {
      id: createId(),
  ...
  async update(tenantId: string, id: string, input: CustomObjectsUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: CustomObjectsRecord = {
      ...current,
      ...input,
  ...
  async save(record: CustomObjectsRecord) {
    records.set(record.id, record);
    return record;
  },
};
