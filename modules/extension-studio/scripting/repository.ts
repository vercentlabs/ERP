...
import type { ScriptingCreateInput, ScriptingListRequest, ScriptingRecord, ScriptingUpdateInput } from "./types";

const records = new Map<string, ScriptingRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const scriptingRepository = {
  async list(request: ScriptingListRequest): Promise<PageResult<ScriptingRecord>> {
  ...
  async create(input: ScriptingCreateInput, actorId = "system"): Promise<ScriptingRecord> {
    const timestamp = now();
    const record: ScriptingRecord = {
      id: createId(),
  ...
  async update(tenantId: string, id: string, input: ScriptingUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: ScriptingRecord = {
      ...current,
      ...input,
  ...
  async save(record: ScriptingRecord) {
    records.set(record.id, record);
    return record;
  },
};
