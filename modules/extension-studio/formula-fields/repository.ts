...
import type { FormulaFieldsCreateInput, FormulaFieldsListRequest, FormulaFieldsRecord, FormulaFieldsUpdateInput } from "./types";

const records = new Map<string, FormulaFieldsRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const formulaFieldsRepository = {
  async list(request: FormulaFieldsListRequest): Promise<PageResult<FormulaFieldsRecord>> {
  ...
  async create(input: FormulaFieldsCreateInput, actorId = "system"): Promise<FormulaFieldsRecord> {
    const timestamp = now();
    const record: FormulaFieldsRecord = {
      id: createId(),
  ...
  async update(tenantId: string, id: string, input: FormulaFieldsUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: FormulaFieldsRecord = {
      ...current,
      ...input,
  ...
  async save(record: FormulaFieldsRecord) {
    records.set(record.id, record);
    return record;
  },
};
