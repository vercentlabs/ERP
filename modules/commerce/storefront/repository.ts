...
import type { StorefrontCreateInput, StorefrontListRequest, StorefrontRecord, StorefrontUpdateInput } from "./types";

const records = new Map<string, StorefrontRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const storefrontRepository = {
  async list(request: StorefrontListRequest): Promise<PageResult<StorefrontRecord>> {
  ...
  async create(input: StorefrontCreateInput, actorId = "system"): Promise<StorefrontRecord> {
    const timestamp = now();
  ...
  async update(tenantId: string, id: string, input: StorefrontUpdateInput, actorId = "system") {
  ...
  async save(record: StorefrontRecord) {
    records.set(record.id, record);
    return record;
  },
};
