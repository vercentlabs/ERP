...
import type { CheckoutCreateInput, CheckoutListRequest, CheckoutRecord, CheckoutUpdateInput } from "./types";
...
export const checkoutRepository = {
  async list(request: CheckoutListRequest): Promise<PageResult<CheckoutRecord>> {
  ...
  async create(input: CheckoutCreateInput, actorId = "system"): Promise<CheckoutRecord> {
    const timestamp = now();
  ...
  async update(tenantId: string, id: string, input: CheckoutUpdateInput, actorId = "system") {
  ...
  async save(record: CheckoutRecord) {
    records.set(record.id, record);
    return record;
  },
};
