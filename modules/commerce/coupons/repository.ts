...
import type { CouponsCreateInput, CouponsListRequest, CouponsRecord, CouponsUpdateInput } from "./types";

const records = new Map<string, CouponsRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const couponsRepository = {
  async list(request: CouponsListRequest): Promise<PageResult<CouponsRecord>> {
  ...
  async create(input: CouponsCreateInput, actorId = "system"): Promise<CouponsRecord> {
    const timestamp = now();
    const record: CouponsRecord = {
      id: createId(),
  ...
  async update(tenantId: string, id: string, input: CouponsUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: CouponsRecord = {
      ...current,
      ...input,
  ...
  async save(record: CouponsRecord) {
    records.set(record.id, record);
    return record;
  },
};
