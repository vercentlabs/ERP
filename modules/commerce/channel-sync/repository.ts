...
import type { ChannelSyncCreateInput, ChannelSyncListRequest, ChannelSyncRecord, ChannelSyncUpdateInput } from "./types";

const records = new Map<string, ChannelSyncRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const channelSyncRepository = {
  async list(request: ChannelSyncListRequest): Promise<PageResult<ChannelSyncRecord>> {
  ...
  async create(input: ChannelSyncCreateInput, actorId = "system"): Promise<ChannelSyncRecord> {
    const timestamp = now();
    const record: ChannelSyncRecord = {
      id: createId(),
  ...
  async update(tenantId: string, id: string, input: ChannelSyncUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: ChannelSyncRecord = {
      ...current,
      ...input,
  ...
  async save(record: ChannelSyncRecord) {
    records.set(record.id, record);
    return record;
  },
};
