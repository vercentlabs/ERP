...
import type { PromotionsCreateInput, PromotionsListRequest, PromotionsRecord, PromotionsUpdateInput } from "./types";

const records = new Map<string, PromotionsRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const promotionsRepository = {
  async list(request: PromotionsListRequest): Promise<PageResult<PromotionsRecord>> {
  ...
  async create(input: PromotionsCreateInput, actorId = "system"): Promise<PromotionsRecord> {
    const timestamp = now();
    const record: PromotionsRecord = {
      id: createId(),
  ...
  async update(tenantId: string, id: string, input: PromotionsUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: PromotionsRecord = {
      ...current,
      ...input,
  ...
  async save(record: PromotionsRecord) {
    records.set(record.id, record);
    return record;
  },
};
