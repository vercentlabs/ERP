...
import type { CustomerGroupsCreateInput, CustomerGroupsListRequest, CustomerGroupsRecord, CustomerGroupsUpdateInput } from "./types";

const records = new Map<string, CustomerGroupsRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const customerGroupsRepository = {
  async list(request: CustomerGroupsListRequest): Promise<PageResult<CustomerGroupsRecord>> {
  ...
  async create(input: CustomerGroupsCreateInput, actorId = "system"): Promise<CustomerGroupsRecord> {
    const timestamp = now();
    const record: CustomerGroupsRecord = {
      id: createId(),
  ...
  async update(tenantId: string, id: string, input: CustomerGroupsUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: CustomerGroupsRecord = {
      ...current,
      ...input,
  ...
  async save(record: CustomerGroupsRecord) {
    records.set(record.id, record);
    return record;
  },
};
