...
import type { CartsCreateInput, CartsListRequest, CartsRecord, CartsUpdateInput } from "./types";

const records = new Map<string, CartsRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const cartsRepository = {
  async list(request: CartsListRequest): Promise<PageResult<CartsRecord>> {
  ...
  async create(input: CartsCreateInput, actorId = "system"): Promise<CartsRecord> {
    const timestamp = now();
    const record: CartsRecord = {
      id: createId(),
  ...
  async update(tenantId: string, id: string, input: CartsUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: CartsRecord = {
      ...current,
      ...input,
