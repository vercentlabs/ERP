...
import type { CommerceOrdersCreateInput, CommerceOrdersListRequest, CommerceOrdersRecord, CommerceOrdersUpdateInput } from "./types";

const records = new Map<string, CommerceOrdersRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const commerceOrdersRepository = {
  async list(request: CommerceOrdersListRequest): Promise<PageResult<CommerceOrdersRecord>> {
  ...
  async create(input: CommerceOrdersCreateInput, actorId = "system"): Promise<CommerceOrdersRecord> {
    const timestamp = now();
    const record: CommerceOrdersRecord = {
      id: createId(),
  ...
  async update(tenantId: string, id: string, input: CommerceOrdersUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: CommerceOrdersRecord = {
      ...current,
      ...input,
  ...
  async save(record: CommerceOrdersRecord) {
    records.set(record.id, record);
    return record;
  },
};
