...
import type { RiskRegisterCreateInput, RiskRegisterListRequest, RiskRegisterRecord, RiskRegisterUpdateInput } from "./types";

const records = new Map<string, RiskRegisterRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const riskRegisterRepository = {
  async list(request: RiskRegisterListRequest): Promise<PageResult<RiskRegisterRecord>> {
  ...
  async create(input: RiskRegisterCreateInput, actorId = "system"): Promise<RiskRegisterRecord> {
    const timestamp = now();
  ...
  async update(tenantId: string, id: string, input: RiskRegisterUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: RiskRegisterRecord = {
      ...current,
      ...input,
  ...
  async save(record: RiskRegisterRecord) {
    records.set(record.id, record);
    return record;
  },
};
