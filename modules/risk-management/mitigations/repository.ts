...
import type { MitigationsCreateInput, MitigationsListRequest, MitigationsRecord, MitigationsUpdateInput } from "./types";

const records = new Map<string, MitigationsRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const mitigationsRepository = {
  async list(request: MitigationsListRequest): Promise<PageResult<MitigationsRecord>> {
  ...
  async create(input: MitigationsCreateInput, actorId = "system"): Promise<MitigationsRecord> {
    const timestamp = now();
    const record: MitigationsRecord = {
      id: createId(),
  ...
  async update(tenantId: string, id: string, input: MitigationsUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: MitigationsRecord = {
      ...current,
      ...input,
  ...
  async save(record: MitigationsRecord) {
    records.set(record.id, record);
    return record;
  },
};
