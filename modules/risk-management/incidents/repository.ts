...
import type { IncidentsCreateInput, IncidentsListRequest, IncidentsRecord, IncidentsUpdateInput } from "./types";

const records = new Map<string, IncidentsRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const incidentsRepository = {
  async list(request: IncidentsListRequest): Promise<PageResult<IncidentsRecord>> {
  ...
  async create(input: IncidentsCreateInput, actorId = "system"): Promise<IncidentsRecord> {
    const timestamp = now();
  ...
  async update(tenantId: string, id: string, input: IncidentsUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: IncidentsRecord = {
      ...current,
      ...input,
  ...
  async save(record: IncidentsRecord) {
    records.set(record.id, record);
    return record;
  },
};
