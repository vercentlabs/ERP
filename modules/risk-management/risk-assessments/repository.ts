...
import type { RiskAssessmentsCreateInput, RiskAssessmentsListRequest, RiskAssessmentsRecord, RiskAssessmentsUpdateInput } from "./types";

const records = new Map<string, RiskAssessmentsRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const riskAssessmentsRepository = {
  async list(request: RiskAssessmentsListRequest): Promise<PageResult<RiskAssessmentsRecord>> {
  ...
  async create(input: RiskAssessmentsCreateInput, actorId = "system"): Promise<RiskAssessmentsRecord> {
    const timestamp = now();
    const record: RiskAssessmentsRecord = {
      id: createId(),
  ...
  async update(tenantId: string, id: string, input: RiskAssessmentsUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: RiskAssessmentsRecord = {
      ...current,
      ...input,
  ...
  async save(record: RiskAssessmentsRecord) {
    records.set(record.id, record);
    return record;
  },
};
