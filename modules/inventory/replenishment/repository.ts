...
import type { ReplenishmentCreateInput, ReplenishmentListRequest, ReplenishmentRecord, ReplenishmentUpdateInput } from "./types";

const records = new Map<string, ReplenishmentRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const replenishmentRepository = {
  async list(request: ReplenishmentListRequest): Promise<PageResult<ReplenishmentRecord>> {
  ...
  async create(input: ReplenishmentCreateInput, actorId = "system"): Promise<ReplenishmentRecord> {
    const timestamp = now();
  ...
  async update(tenantId: string, id: string, input: ReplenishmentUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: ReplenishmentRecord = {
      ...current,
      ...input,
  ...
  async save(record: ReplenishmentRecord) {
    records.set(record.id, record);
    return record;
  },
};
