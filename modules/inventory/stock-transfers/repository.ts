...
import type { StockTransfersCreateInput, StockTransfersListRequest, StockTransfersRecord, StockTransfersUpdateInput } from "./types";

const records = new Map<string, StockTransfersRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const stockTransfersRepository = {
  async list(request: StockTransfersListRequest): Promise<PageResult<StockTransfersRecord>> {
  ...
  async create(input: StockTransfersCreateInput, actorId = "system"): Promise<StockTransfersRecord> {
    const timestamp = now();
    const record: StockTransfersRecord = {
      id: createId(),
  ...
  async update(tenantId: string, id: string, input: StockTransfersUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: StockTransfersRecord = {
      ...current,
      ...input,
  ...
  async save(record: StockTransfersRecord) {
    records.set(record.id, record);
    return record;
  },
};
