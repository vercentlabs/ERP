...
import type { ItemsCreateInput, ItemsListRequest, ItemsRecord, ItemsUpdateInput } from "./types";

const records = new Map<string, ItemsRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const itemsRepository = {
  async list(request: ItemsListRequest): Promise<PageResult<ItemsRecord>> {
  ...
  async create(input: ItemsCreateInput, actorId = "system"): Promise<ItemsRecord> {
    const timestamp = now();
  ...
  async update(tenantId: string, id: string, input: ItemsUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: ItemsRecord = {
      ...current,
      ...input,
