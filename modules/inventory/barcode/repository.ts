...
import type { BarcodeCreateInput, BarcodeListRequest, BarcodeRecord, BarcodeUpdateInput } from "./types";

const records = new Map<string, BarcodeRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const barcodeRepository = {
  async list(request: BarcodeListRequest): Promise<PageResult<BarcodeRecord>> {
  ...
  async create(input: BarcodeCreateInput, actorId = "system"): Promise<BarcodeRecord> {
    const timestamp = now();
  ...
  async update(tenantId: string, id: string, input: BarcodeUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: BarcodeRecord = {
      ...current,
      ...input,
  ...
  async save(record: BarcodeRecord) {
    records.set(record.id, record);
    return record;
  },
};
