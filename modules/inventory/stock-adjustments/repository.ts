...
import type { StockAdjustmentsCreateInput, StockAdjustmentsListRequest, StockAdjustmentsRecord, StockAdjustmentsUpdateInput } from "./types";

const records = new Map<string, StockAdjustmentsRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const stockAdjustmentsRepository = {
  async list(request: StockAdjustmentsListRequest): Promise<PageResult<StockAdjustmentsRecord>> {
  ...
  async create(input: StockAdjustmentsCreateInput, actorId = "system"): Promise<StockAdjustmentsRecord> {
    const timestamp = now();
    const record: StockAdjustmentsRecord = {
      id: createId(),
  ...
  async update(tenantId: string, id: string, input: StockAdjustmentsUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: StockAdjustmentsRecord = {
      ...current,
      ...input,
  ...
  async save(record: StockAdjustmentsRecord) {
    records.set(record.id, record);
    return record;
  },
};
