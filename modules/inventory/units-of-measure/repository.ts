...
import type { UnitsOfMeasureCreateInput, UnitsOfMeasureListRequest, UnitsOfMeasureRecord, UnitsOfMeasureUpdateInput } from "./types";

const records = new Map<string, UnitsOfMeasureRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const unitsOfMeasureRepository = {
  async list(request: UnitsOfMeasureListRequest): Promise<PageResult<UnitsOfMeasureRecord>> {
  ...
  async create(input: UnitsOfMeasureCreateInput, actorId = "system"): Promise<UnitsOfMeasureRecord> {
    const timestamp = now();
    const record: UnitsOfMeasureRecord = {
      id: createId(),
  ...
  async update(tenantId: string, id: string, input: UnitsOfMeasureUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: UnitsOfMeasureRecord = {
      ...current,
      ...input,
  ...
  async save(record: UnitsOfMeasureRecord) {
    records.set(record.id, record);
    return record;
  },
};
