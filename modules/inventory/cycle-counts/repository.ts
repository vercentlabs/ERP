...
import type { CycleCountsCreateInput, CycleCountsListRequest, CycleCountsRecord, CycleCountsUpdateInput } from "./types";

const records = new Map<string, CycleCountsRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const cycleCountsRepository = {
  async list(request: CycleCountsListRequest): Promise<PageResult<CycleCountsRecord>> {
  ...
  async create(input: CycleCountsCreateInput, actorId = "system"): Promise<CycleCountsRecord> {
    const timestamp = now();
    const record: CycleCountsRecord = {
      id: createId(),
  ...
  async update(tenantId: string, id: string, input: CycleCountsUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: CycleCountsRecord = {
      ...current,
      ...input,
  ...
  async save(record: CycleCountsRecord) {
    records.set(record.id, record);
    return record;
  },
};
