...
import type { ValuationCreateInput, ValuationListRequest, ValuationRecord, ValuationUpdateInput } from "./types";

const records = new Map<string, ValuationRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const valuationRepository = {
  async list(request: ValuationListRequest): Promise<PageResult<ValuationRecord>> {
  ...
  async create(input: ValuationCreateInput, actorId = "system"): Promise<ValuationRecord> {
    const timestamp = now();
  ...
  async update(tenantId: string, id: string, input: ValuationUpdateInput, actorId = "system") {
  ...
  async save(record: ValuationRecord) {
    records.set(record.id, record);
    return record;
  },
};
