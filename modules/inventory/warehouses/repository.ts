...
import type { WarehousesCreateInput, WarehousesListRequest, WarehousesRecord, WarehousesUpdateInput } from "./types";

const records = new Map<string, WarehousesRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const warehousesRepository = {
  async list(request: WarehousesListRequest): Promise<PageResult<WarehousesRecord>> {
  ...
  async create(input: WarehousesCreateInput, actorId = "system"): Promise<WarehousesRecord> {
    const timestamp = now();
    const record: WarehousesRecord = {
      id: createId(),
  ...
  async update(tenantId: string, id: string, input: WarehousesUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: WarehousesRecord = {
      ...current,
      ...input,
  ...
  async save(record: WarehousesRecord) {
    records.set(record.id, record);
    return record;
  },
};
