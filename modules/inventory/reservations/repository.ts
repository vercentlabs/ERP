...
import type { ReservationsCreateInput, ReservationsListRequest, ReservationsRecord, ReservationsUpdateInput } from "./types";

const records = new Map<string, ReservationsRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const reservationsRepository = {
  async list(request: ReservationsListRequest): Promise<PageResult<ReservationsRecord>> {
  ...
  async create(input: ReservationsCreateInput, actorId = "system"): Promise<ReservationsRecord> {
    const timestamp = now();
    const record: ReservationsRecord = {
      id: createId(),
  ...
  async update(tenantId: string, id: string, input: ReservationsUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: ReservationsRecord = {
      ...current,
      ...input,
  ...
  async save(record: ReservationsRecord) {
    records.set(record.id, record);
    return record;
  },
};
