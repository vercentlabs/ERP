...
import type { ItemCategoriesCreateInput, ItemCategoriesListRequest, ItemCategoriesRecord, ItemCategoriesUpdateInput } from "./types";

const records = new Map<string, ItemCategoriesRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const itemCategoriesRepository = {
  async list(request: ItemCategoriesListRequest): Promise<PageResult<ItemCategoriesRecord>> {
  ...
  async create(input: ItemCategoriesCreateInput, actorId = "system"): Promise<ItemCategoriesRecord> {
    const timestamp = now();
    const record: ItemCategoriesRecord = {
      id: createId(),
  ...
  async update(tenantId: string, id: string, input: ItemCategoriesUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: ItemCategoriesRecord = {
      ...current,
      ...input,
  ...
  async save(record: ItemCategoriesRecord) {
    records.set(record.id, record);
    return record;
  },
};
