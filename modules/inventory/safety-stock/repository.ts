...
import type { SafetyStockCreateInput, SafetyStockListRequest, SafetyStockRecord, SafetyStockUpdateInput } from "./types";

const records = new Map<string, SafetyStockRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const safetyStockRepository = {
  async list(request: SafetyStockListRequest): Promise<PageResult<SafetyStockRecord>> {
  ...
  async create(input: SafetyStockCreateInput, actorId = "system"): Promise<SafetyStockRecord> {
    const timestamp = now();
    const record: SafetyStockRecord = {
      id: createId(),
  ...
  async update(tenantId: string, id: string, input: SafetyStockUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: SafetyStockRecord = {
      ...current,
      ...input,
  ...
  async save(record: SafetyStockRecord) {
    records.set(record.id, record);
    return record;
  },
};
