...
import type { LocationsCreateInput, LocationsListRequest, LocationsRecord, LocationsUpdateInput } from "./types";

const records = new Map<string, LocationsRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const locationsRepository = {
  async list(request: LocationsListRequest): Promise<PageResult<LocationsRecord>> {
  ...
  async create(input: LocationsCreateInput, actorId = "system"): Promise<LocationsRecord> {
    const timestamp = now();
    const record: LocationsRecord = {
      id: createId(),
  ...
  async update(tenantId: string, id: string, input: LocationsUpdateInput, actorId = "system") {
  ...
  async save(record: LocationsRecord) {
    records.set(record.id, record);
    return record;
  },
};
