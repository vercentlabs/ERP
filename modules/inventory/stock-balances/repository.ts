...
import type { StockBalancesCreateInput, StockBalancesListRequest, StockBalancesRecord, StockBalancesUpdateInput } from "./types";

const records = new Map<string, StockBalancesRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const stockBalancesRepository = {
  async list(request: StockBalancesListRequest): Promise<PageResult<StockBalancesRecord>> {
  ...
  async create(input: StockBalancesCreateInput, actorId = "system"): Promise<StockBalancesRecord> {
    const timestamp = now();
    const record: StockBalancesRecord = {
      id: createId(),
  ...
  async update(tenantId: string, id: string, input: StockBalancesUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: StockBalancesRecord = {
      ...current,
      ...input,
  ...
  async save(record: StockBalancesRecord) {
    records.set(record.id, record);
    return record;
  },
};
