...
import type { DemandPlanningCreateInput, DemandPlanningListRequest, DemandPlanningRecord, DemandPlanningUpdateInput } from "./types";

const records = new Map<string, DemandPlanningRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const demandPlanningRepository = {
  async list(request: DemandPlanningListRequest): Promise<PageResult<DemandPlanningRecord>> {
  ...
  async create(input: DemandPlanningCreateInput, actorId = "system"): Promise<DemandPlanningRecord> {
    const timestamp = now();
    const record: DemandPlanningRecord = {
      id: createId(),
  ...
  async update(tenantId: string, id: string, input: DemandPlanningUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: DemandPlanningRecord = {
      ...current,
      ...input,
  ...
  async save(record: DemandPlanningRecord) {
    records.set(record.id, record);
    return record;
  },
};
