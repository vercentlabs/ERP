...
import type { SerialBatchesCreateInput, SerialBatchesListRequest, SerialBatchesRecord, SerialBatchesUpdateInput } from "./types";

const records = new Map<string, SerialBatchesRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const serialBatchesRepository = {
  async list(request: SerialBatchesListRequest): Promise<PageResult<SerialBatchesRecord>> {
  ...
  async create(input: SerialBatchesCreateInput, actorId = "system"): Promise<SerialBatchesRecord> {
    const timestamp = now();
    const record: SerialBatchesRecord = {
      id: createId(),
  ...
  async update(tenantId: string, id: string, input: SerialBatchesUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: SerialBatchesRecord = {
      ...current,
      ...input,
  ...
  async save(record: SerialBatchesRecord) {
    records.set(record.id, record);
    return record;
  },
};
