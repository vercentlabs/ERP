...
import type { VarianceAnalysisCreateInput, VarianceAnalysisListRequest, VarianceAnalysisRecord, VarianceAnalysisUpdateInput } from "./types";

const records = new Map<string, VarianceAnalysisRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const varianceAnalysisRepository = {
  async list(request: VarianceAnalysisListRequest): Promise<PageResult<VarianceAnalysisRecord>> {
  ...
  async create(input: VarianceAnalysisCreateInput, actorId = "system"): Promise<VarianceAnalysisRecord> {
    const timestamp = now();
  ...
  async update(tenantId: string, id: string, input: VarianceAnalysisUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: VarianceAnalysisRecord = {
      ...current,
      ...input,
  ...
  async save(record: VarianceAnalysisRecord) {
    records.set(record.id, record);
    return record;
  },
};
