...
import type { BudgetingCreateInput, BudgetingListRequest, BudgetingRecord, BudgetingUpdateInput } from "./types";

const records = new Map<string, BudgetingRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const budgetingRepository = {
  async list(request: BudgetingListRequest): Promise<PageResult<BudgetingRecord>> {
  ...
  async create(input: BudgetingCreateInput, actorId = "system"): Promise<BudgetingRecord> {
    const timestamp = now();
    const record: BudgetingRecord = {
      id: createId(),
  ...
  async update(tenantId: string, id: string, input: BudgetingUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: BudgetingRecord = {
      ...current,
      ...input,
  ...
  async save(record: BudgetingRecord) {
    records.set(record.id, record);
    return record;
  },
};
