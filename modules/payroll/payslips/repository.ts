...
import type { PayslipsCreateInput, PayslipsListRequest, PayslipsRecord, PayslipsUpdateInput } from "./types";

const records = new Map<string, PayslipsRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const payslipsRepository = {
  async list(request: PayslipsListRequest): Promise<PageResult<PayslipsRecord>> {
  ...
  async create(input: PayslipsCreateInput, actorId = "system"): Promise<PayslipsRecord> {
    const timestamp = now();
    const record: PayslipsRecord = {
      id: createId(),
  ...
  async update(tenantId: string, id: string, input: PayslipsUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: PayslipsRecord = {
      ...current,
      ...input,
  ...
  async save(record: PayslipsRecord) {
    records.set(record.id, record);
    return record;
  },
};
