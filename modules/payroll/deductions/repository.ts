...
import type { DeductionsCreateInput, DeductionsListRequest, DeductionsRecord, DeductionsUpdateInput } from "./types";

const records = new Map<string, DeductionsRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const deductionsRepository = {
  async list(request: DeductionsListRequest): Promise<PageResult<DeductionsRecord>> {
  ...
  async create(input: DeductionsCreateInput, actorId = "system"): Promise<DeductionsRecord> {
    const timestamp = now();
    const record: DeductionsRecord = {
      id: createId(),
  ...
  async update(tenantId: string, id: string, input: DeductionsUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: DeductionsRecord = {
      ...current,
      ...input,
  ...
  async save(record: DeductionsRecord) {
    records.set(record.id, record);
    return record;
  },
};
