...
import type { PayrollRunsCreateInput, PayrollRunsListRequest, PayrollRunsRecord, PayrollRunsUpdateInput } from "./types";

const records = new Map<string, PayrollRunsRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const payrollRunsRepository = {
  async list(request: PayrollRunsListRequest): Promise<PageResult<PayrollRunsRecord>> {
  ...
  async create(input: PayrollRunsCreateInput, actorId = "system"): Promise<PayrollRunsRecord> {
    const timestamp = now();
    const record: PayrollRunsRecord = {
      id: createId(),
  ...
  async update(tenantId: string, id: string, input: PayrollRunsUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: PayrollRunsRecord = {
      ...current,
      ...input,
  ...
  async save(record: PayrollRunsRecord) {
    records.set(record.id, record);
    return record;
  },
};
