...
import type { PayrollPeriodsCreateInput, PayrollPeriodsListRequest, PayrollPeriodsRecord, PayrollPeriodsUpdateInput } from "./types";

const records = new Map<string, PayrollPeriodsRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const payrollPeriodsRepository = {
  async list(request: PayrollPeriodsListRequest): Promise<PageResult<PayrollPeriodsRecord>> {
  ...
  async create(input: PayrollPeriodsCreateInput, actorId = "system"): Promise<PayrollPeriodsRecord> {
    const timestamp = now();
    const record: PayrollPeriodsRecord = {
      id: createId(),
  ...
  async update(tenantId: string, id: string, input: PayrollPeriodsUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: PayrollPeriodsRecord = {
      ...current,
      ...input,
  ...
  async save(record: PayrollPeriodsRecord) {
    records.set(record.id, record);
    return record;
  },
};
