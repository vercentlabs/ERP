...
import type { LoansAdvancesCreateInput, LoansAdvancesListRequest, LoansAdvancesRecord, LoansAdvancesUpdateInput } from "./types";

const records = new Map<string, LoansAdvancesRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const loansAdvancesRepository = {
  async list(request: LoansAdvancesListRequest): Promise<PageResult<LoansAdvancesRecord>> {
  ...
  async create(input: LoansAdvancesCreateInput, actorId = "system"): Promise<LoansAdvancesRecord> {
    const timestamp = now();
    const record: LoansAdvancesRecord = {
      id: createId(),
  ...
  async update(tenantId: string, id: string, input: LoansAdvancesUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: LoansAdvancesRecord = {
      ...current,
      ...input,
  ...
  async save(record: LoansAdvancesRecord) {
    records.set(record.id, record);
    return record;
  },
};
