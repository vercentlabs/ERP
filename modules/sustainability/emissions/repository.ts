...
import type { EmissionsCreateInput, EmissionsListRequest, EmissionsRecord, EmissionsUpdateInput } from "./types";

const records = new Map<string, EmissionsRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const emissionsRepository = {
  async list(request: EmissionsListRequest): Promise<PageResult<EmissionsRecord>> {
  ...
  async create(input: EmissionsCreateInput, actorId = "system"): Promise<EmissionsRecord> {
    const timestamp = now();
    const record: EmissionsRecord = {
      id: createId(),
  ...
  async update(tenantId: string, id: string, input: EmissionsUpdateInput, actorId = "system") {
  ...
  async save(record: EmissionsRecord) {
    records.set(record.id, record);
    return record;
  },
};
