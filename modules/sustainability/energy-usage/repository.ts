...
import type { EnergyUsageCreateInput, EnergyUsageListRequest, EnergyUsageRecord, EnergyUsageUpdateInput } from "./types";

const records = new Map<string, EnergyUsageRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const energyUsageRepository = {
  async list(request: EnergyUsageListRequest): Promise<PageResult<EnergyUsageRecord>> {
  ...
  async create(input: EnergyUsageCreateInput, actorId = "system"): Promise<EnergyUsageRecord> {
    const timestamp = now();
  ...
  async update(tenantId: string, id: string, input: EnergyUsageUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: EnergyUsageRecord = {
      ...current,
      ...input,
  ...
  async save(record: EnergyUsageRecord) {
    records.set(record.id, record);
    return record;
  },
};
