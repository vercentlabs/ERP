...
import type { DesignationsCreateInput, DesignationsListRequest, DesignationsRecord, DesignationsUpdateInput } from "./types";

const records = new Map<string, DesignationsRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const designationsRepository = {
  async list(request: DesignationsListRequest): Promise<PageResult<DesignationsRecord>> {
  ...
  async create(input: DesignationsCreateInput, actorId = "system"): Promise<DesignationsRecord> {
    const timestamp = now();
    const record: DesignationsRecord = {
      id: createId(),
  ...
  async update(tenantId: string, id: string, input: DesignationsUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: DesignationsRecord = {
      ...current,
      ...input,
  ...
  async save(record: DesignationsRecord) {
    records.set(record.id, record);
    return record;
  },
};
