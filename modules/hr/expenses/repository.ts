...
import type { ExpensesCreateInput, ExpensesListRequest, ExpensesRecord, ExpensesUpdateInput } from "./types";

const records = new Map<string, ExpensesRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const expensesRepository = {
  async list(request: ExpensesListRequest): Promise<PageResult<ExpensesRecord>> {
  ...
  async create(input: ExpensesCreateInput, actorId = "system"): Promise<ExpensesRecord> {
    const timestamp = now();
    const record: ExpensesRecord = {
      id: createId(),
  ...
  async update(tenantId: string, id: string, input: ExpensesUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: ExpensesRecord = {
      ...current,
      ...input,
  ...
  async save(record: ExpensesRecord) {
    records.set(record.id, record);
    return record;
  },
};
