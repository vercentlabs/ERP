...
import type { ShiftsCreateInput, ShiftsListRequest, ShiftsRecord, ShiftsUpdateInput } from "./types";

const records = new Map<string, ShiftsRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const shiftsRepository = {
  async list(request: ShiftsListRequest): Promise<PageResult<ShiftsRecord>> {
  ...
  async create(input: ShiftsCreateInput, actorId = "system"): Promise<ShiftsRecord> {
    const timestamp = now();
    const record: ShiftsRecord = {
      id: createId(),
  ...
  async update(tenantId: string, id: string, input: ShiftsUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: ShiftsRecord = {
      ...current,
      ...input,
  ...
  async save(record: ShiftsRecord) {
    records.set(record.id, record);
    return record;
  },
};
