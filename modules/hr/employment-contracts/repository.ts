...
import type { EmploymentContractsCreateInput, EmploymentContractsListRequest, EmploymentContractsRecord, EmploymentContractsUpdateInput } from "./types";

const records = new Map<string, EmploymentContractsRecord>();
...
export const employmentContractsRepository = {
  async list(request: EmploymentContractsListRequest): Promise<PageResult<EmploymentContractsRecord>> {
  ...
  async create(input: EmploymentContractsCreateInput, actorId = "system"): Promise<EmploymentContractsRecord> {
    const timestamp = now();
    const record: EmploymentContractsRecord = {
      id: createId(),
  ...
  async update(tenantId: string, id: string, input: EmploymentContractsUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: EmploymentContractsRecord = {
      ...current,
      ...input,
  ...
  async save(record: EmploymentContractsRecord) {
    records.set(record.id, record);
    return record;
  },
};
