...
import type { RecruitmentCreateInput, RecruitmentListRequest, RecruitmentRecord, RecruitmentUpdateInput } from "./types";

const records = new Map<string, RecruitmentRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const recruitmentRepository = {
  async list(request: RecruitmentListRequest): Promise<PageResult<RecruitmentRecord>> {
  ...
  async create(input: RecruitmentCreateInput, actorId = "system"): Promise<RecruitmentRecord> {
    const timestamp = now();
  ...
  async update(tenantId: string, id: string, input: RecruitmentUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: RecruitmentRecord = {
      ...current,
      ...input,
  ...
  async save(record: RecruitmentRecord) {
    records.set(record.id, record);
    return record;
  },
};
