...
import type { DepartmentsCreateInput, DepartmentsListRequest, DepartmentsRecord, DepartmentsUpdateInput } from "./types";

const records = new Map<string, DepartmentsRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const departmentsRepository = {
  async list(request: DepartmentsListRequest): Promise<PageResult<DepartmentsRecord>> {
  ...
  async create(input: DepartmentsCreateInput, actorId = "system"): Promise<DepartmentsRecord> {
    const timestamp = now();
    const record: DepartmentsRecord = {
      id: createId(),
  ...
  async update(tenantId: string, id: string, input: DepartmentsUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: DepartmentsRecord = {
      ...current,
      ...input,
  ...
  async save(record: DepartmentsRecord) {
    records.set(record.id, record);
    return record;
  },
};
