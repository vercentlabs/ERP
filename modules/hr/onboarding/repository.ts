...
import type { OnboardingCreateInput, OnboardingListRequest, OnboardingRecord, OnboardingUpdateInput } from "./types";
...
export const onboardingRepository = {
  async list(request: OnboardingListRequest): Promise<PageResult<OnboardingRecord>> {
  ...
  async create(input: OnboardingCreateInput, actorId = "system"): Promise<OnboardingRecord> {
    const timestamp = now();
  ...
  async update(tenantId: string, id: string, input: OnboardingUpdateInput, actorId = "system") {
  ...
  async save(record: OnboardingRecord) {
    records.set(record.id, record);
    return record;
  },
};
