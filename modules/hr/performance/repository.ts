...
import type { PerformanceCreateInput, PerformanceListRequest, PerformanceRecord, PerformanceUpdateInput } from "./types";
...
export const performanceRepository = {
  async list(request: PerformanceListRequest): Promise<PageResult<PerformanceRecord>> {
  ...
  async create(input: PerformanceCreateInput, actorId = "system"): Promise<PerformanceRecord> {
    const timestamp = now();
  ...
  async update(tenantId: string, id: string, input: PerformanceUpdateInput, actorId = "system") {
  ...
  async save(record: PerformanceRecord) {
    records.set(record.id, record);
    return record;
  },
};
