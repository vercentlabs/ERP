...
import type { DispatchCreateInput, DispatchListRequest, DispatchRecord, DispatchUpdateInput } from "./types";
...
export const dispatchRepository = {
  async list(request: DispatchListRequest): Promise<PageResult<DispatchRecord>> {
  ...
  async create(input: DispatchCreateInput, actorId = "system"): Promise<DispatchRecord> {
    const timestamp = now();
  ...
  async update(tenantId: string, id: string, input: DispatchUpdateInput, actorId = "system") {
  ...
  async save(record: DispatchRecord) {
    records.set(record.id, record);
    return record;
  },
};
