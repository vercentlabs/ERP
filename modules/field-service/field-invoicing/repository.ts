...
import type { FieldInvoicingCreateInput, FieldInvoicingListRequest, FieldInvoicingRecord, FieldInvoicingUpdateInput } from "./types";

const records = new Map<string, FieldInvoicingRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const fieldInvoicingRepository = {
  async list(request: FieldInvoicingListRequest): Promise<PageResult<FieldInvoicingRecord>> {
  ...
  async create(input: FieldInvoicingCreateInput, actorId = "system"): Promise<FieldInvoicingRecord> {
    const timestamp = now();
    const record: FieldInvoicingRecord = {
      id: createId(),
  ...
  async update(tenantId: string, id: string, input: FieldInvoicingUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: FieldInvoicingRecord = {
      ...current,
      ...input,
  ...
  async save(record: FieldInvoicingRecord) {
    records.set(record.id, record);
    return record;
  },
};
