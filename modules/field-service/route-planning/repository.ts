...
import type { RoutePlanningCreateInput, RoutePlanningListRequest, RoutePlanningRecord, RoutePlanningUpdateInput } from "./types";

const records = new Map<string, RoutePlanningRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const routePlanningRepository = {
  async list(request: RoutePlanningListRequest): Promise<PageResult<RoutePlanningRecord>> {
  ...
  async create(input: RoutePlanningCreateInput, actorId = "system"): Promise<RoutePlanningRecord> {
    const timestamp = now();
    const record: RoutePlanningRecord = {
      id: createId(),
  ...
  async update(tenantId: string, id: string, input: RoutePlanningUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: RoutePlanningRecord = {
      ...current,
      ...input,
  ...
  async save(record: RoutePlanningRecord) {
    records.set(record.id, record);
    return record;
  },
};
