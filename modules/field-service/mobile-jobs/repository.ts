...
import type { MobileJobsCreateInput, MobileJobsListRequest, MobileJobsRecord, MobileJobsUpdateInput } from "./types";

const records = new Map<string, MobileJobsRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const mobileJobsRepository = {
  async list(request: MobileJobsListRequest): Promise<PageResult<MobileJobsRecord>> {
  ...
  async create(input: MobileJobsCreateInput, actorId = "system"): Promise<MobileJobsRecord> {
    const timestamp = now();
    const record: MobileJobsRecord = {
      id: createId(),
  ...
  async update(tenantId: string, id: string, input: MobileJobsUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: MobileJobsRecord = {
      ...current,
      ...input,
  ...
  async save(record: MobileJobsRecord) {
    records.set(record.id, record);
    return record;
  },
};
