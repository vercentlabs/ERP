...
import type { TechniciansCreateInput, TechniciansListRequest, TechniciansRecord, TechniciansUpdateInput } from "./types";

const records = new Map<string, TechniciansRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const techniciansRepository = {
  async list(request: TechniciansListRequest): Promise<PageResult<TechniciansRecord>> {
  ...
  async create(input: TechniciansCreateInput, actorId = "system"): Promise<TechniciansRecord> {
    const timestamp = now();
    const record: TechniciansRecord = {
      id: createId(),
  ...
  async update(tenantId: string, id: string, input: TechniciansUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: TechniciansRecord = {
      ...current,
      ...input,
  ...
  async save(record: TechniciansRecord) {
    records.set(record.id, record);
    return record;
  },
};
