...
import type { ServicePartsCreateInput, ServicePartsListRequest, ServicePartsRecord, ServicePartsUpdateInput } from "./types";

const records = new Map<string, ServicePartsRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const servicePartsRepository = {
  async list(request: ServicePartsListRequest): Promise<PageResult<ServicePartsRecord>> {
  ...
  async create(input: ServicePartsCreateInput, actorId = "system"): Promise<ServicePartsRecord> {
    const timestamp = now();
    const record: ServicePartsRecord = {
      id: createId(),
  ...
  async update(tenantId: string, id: string, input: ServicePartsUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: ServicePartsRecord = {
      ...current,
      ...input,
  ...
  async save(record: ServicePartsRecord) {
    records.set(record.id, record);
    return record;
  },
};
