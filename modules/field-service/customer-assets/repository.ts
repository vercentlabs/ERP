...
import type { CustomerAssetsCreateInput, CustomerAssetsListRequest, CustomerAssetsRecord, CustomerAssetsUpdateInput } from "./types";

const records = new Map<string, CustomerAssetsRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const customerAssetsRepository = {
  async list(request: CustomerAssetsListRequest): Promise<PageResult<CustomerAssetsRecord>> {
  ...
  async create(input: CustomerAssetsCreateInput, actorId = "system"): Promise<CustomerAssetsRecord> {
    const timestamp = now();
    const record: CustomerAssetsRecord = {
      id: createId(),
  ...
  async update(tenantId: string, id: string, input: CustomerAssetsUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: CustomerAssetsRecord = {
      ...current,
      ...input,
  ...
  async save(record: CustomerAssetsRecord) {
    records.set(record.id, record);
    return record;
  },
};
