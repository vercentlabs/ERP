...
import type { EquipmentCreateInput, EquipmentListRequest, EquipmentRecord, EquipmentUpdateInput } from "./types";
...
export const equipmentRepository = {
  async list(request: EquipmentListRequest): Promise<PageResult<EquipmentRecord>> {
  ...
  async create(input: EquipmentCreateInput, actorId = "system"): Promise<EquipmentRecord> {
    const timestamp = now();
  ...
  async update(tenantId: string, id: string, input: EquipmentUpdateInput, actorId = "system") {
  ...
  async save(record: EquipmentRecord) {
    records.set(record.id, record);
    return record;
  },
};
