...
import type { SparePartsCreateInput, SparePartsListRequest, SparePartsRecord, SparePartsUpdateInput } from "./types";

const records = new Map<string, SparePartsRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const sparePartsRepository = {
  async list(request: SparePartsListRequest): Promise<PageResult<SparePartsRecord>> {
  ...
  async create(input: SparePartsCreateInput, actorId = "system"): Promise<SparePartsRecord> {
    const timestamp = now();
    const record: SparePartsRecord = {
      id: createId(),
  ...
  async update(tenantId: string, id: string, input: SparePartsUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: SparePartsRecord = {
      ...current,
      ...input,
  ...
  async save(record: SparePartsRecord) {
    records.set(record.id, record);
    return record;
  },
};
