...
import type { DowntimeCreateInput, DowntimeListRequest, DowntimeRecord, DowntimeUpdateInput } from "./types";

const records = new Map<string, DowntimeRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const downtimeRepository = {
  async list(request: DowntimeListRequest): Promise<PageResult<DowntimeRecord>> {
  ...
  async create(input: DowntimeCreateInput, actorId = "system"): Promise<DowntimeRecord> {
    const timestamp = now();
  ...
  async update(tenantId: string, id: string, input: DowntimeUpdateInput, actorId = "system") {
  ...
  async save(record: DowntimeRecord) {
    records.set(record.id, record);
    return record;
  },
};
