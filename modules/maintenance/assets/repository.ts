...
import type { AssetsCreateInput, AssetsListRequest, AssetsRecord, AssetsUpdateInput } from "./types";

const records = new Map<string, AssetsRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const assetsRepository = {
  async list(request: AssetsListRequest): Promise<PageResult<AssetsRecord>> {
  ...
  async create(input: AssetsCreateInput, actorId = "system"): Promise<AssetsRecord> {
    const timestamp = now();
  ...
  async update(tenantId: string, id: string, input: AssetsUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: AssetsRecord = {
      ...current,
      ...input,
  ...
  async save(record: AssetsRecord) {
    records.set(record.id, record);
    return record;
  },
};
