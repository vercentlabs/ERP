...
import type { AssetPerformanceCreateInput, AssetPerformanceListRequest, AssetPerformanceRecord, AssetPerformanceUpdateInput } from "./types";

const records = new Map<string, AssetPerformanceRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const assetPerformanceRepository = {
  async list(request: AssetPerformanceListRequest): Promise<PageResult<AssetPerformanceRecord>> {
  ...
  async create(input: AssetPerformanceCreateInput, actorId = "system"): Promise<AssetPerformanceRecord> {
    const timestamp = now();
  ...
  async update(tenantId: string, id: string, input: AssetPerformanceUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: AssetPerformanceRecord = {
      ...current,
      ...input,
  ...
  async save(record: AssetPerformanceRecord) {
    records.set(record.id, record);
    return record;
  },
};
