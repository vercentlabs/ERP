...
import type { WorkCentersCreateInput, WorkCentersListRequest, WorkCentersRecord, WorkCentersUpdateInput } from "./types";

const records = new Map<string, WorkCentersRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const workCentersRepository = {
  async list(request: WorkCentersListRequest): Promise<PageResult<WorkCentersRecord>> {
  ...
  async create(input: WorkCentersCreateInput, actorId = "system"): Promise<WorkCentersRecord> {
    const timestamp = now();
    const record: WorkCentersRecord = {
      id: createId(),
  ...
  async update(tenantId: string, id: string, input: WorkCentersUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: WorkCentersRecord = {
      ...current,
      ...input,
  ...
  async save(record: WorkCentersRecord) {
    records.set(record.id, record);
    return record;
  },
};
