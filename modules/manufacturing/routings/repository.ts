...
import type { RoutingsCreateInput, RoutingsListRequest, RoutingsRecord, RoutingsUpdateInput } from "./types";

const records = new Map<string, RoutingsRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const routingsRepository = {
  async list(request: RoutingsListRequest): Promise<PageResult<RoutingsRecord>> {
  ...
  async create(input: RoutingsCreateInput, actorId = "system"): Promise<RoutingsRecord> {
    const timestamp = now();
    const record: RoutingsRecord = {
      id: createId(),
  ...
  async update(tenantId: string, id: string, input: RoutingsUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: RoutingsRecord = {
      ...current,
      ...input,
  ...
  async save(record: RoutingsRecord) {
    records.set(record.id, record);
    return record;
  },
};
