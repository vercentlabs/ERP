...
import type { ScrapReworkCreateInput, ScrapReworkListRequest, ScrapReworkRecord, ScrapReworkUpdateInput } from "./types";

const records = new Map<string, ScrapReworkRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const scrapReworkRepository = {
  async list(request: ScrapReworkListRequest): Promise<PageResult<ScrapReworkRecord>> {
  ...
  async create(input: ScrapReworkCreateInput, actorId = "system"): Promise<ScrapReworkRecord> {
    const timestamp = now();
  ...
  async update(tenantId: string, id: string, input: ScrapReworkUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: ScrapReworkRecord = {
      ...current,
      ...input,
  ...
  async save(record: ScrapReworkRecord) {
    records.set(record.id, record);
    return record;
  },
};
