...
import type { SubcontractingCreateInput, SubcontractingListRequest, SubcontractingRecord, SubcontractingUpdateInput } from "./types";

const records = new Map<string, SubcontractingRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const subcontractingRepository = {
  async list(request: SubcontractingListRequest): Promise<PageResult<SubcontractingRecord>> {
  ...
  async create(input: SubcontractingCreateInput, actorId = "system"): Promise<SubcontractingRecord> {
    const timestamp = now();
  ...
  async update(tenantId: string, id: string, input: SubcontractingUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: SubcontractingRecord = {
      ...current,
      ...input,
  ...
  async save(record: SubcontractingRecord) {
    records.set(record.id, record);
    return record;
  },
};
