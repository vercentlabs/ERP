...
import type { ShopFloorCreateInput, ShopFloorListRequest, ShopFloorRecord, ShopFloorUpdateInput } from "./types";

const records = new Map<string, ShopFloorRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const shopFloorRepository = {
  async list(request: ShopFloorListRequest): Promise<PageResult<ShopFloorRecord>> {
  ...
  async create(input: ShopFloorCreateInput, actorId = "system"): Promise<ShopFloorRecord> {
    const timestamp = now();
    const record: ShopFloorRecord = {
      id: createId(),
  ...
  async update(tenantId: string, id: string, input: ShopFloorUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: ShopFloorRecord = {
      ...current,
      ...input,
  ...
  async save(record: ShopFloorRecord) {
    records.set(record.id, record);
    return record;
  },
};
