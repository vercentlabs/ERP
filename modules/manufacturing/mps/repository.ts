...
import type { MpsCreateInput, MpsListRequest, MpsRecord, MpsUpdateInput } from "./types";

const records = new Map<string, MpsRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const mpsRepository = {
  async list(request: MpsListRequest): Promise<PageResult<MpsRecord>> {
  ...
  async create(input: MpsCreateInput, actorId = "system"): Promise<MpsRecord> {
    const timestamp = now();
    const record: MpsRecord = {
      id: createId(),
  ...
  async update(tenantId: string, id: string, input: MpsUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: MpsRecord = {
      ...current,
      ...input,
