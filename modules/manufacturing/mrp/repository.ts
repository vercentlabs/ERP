...
import type { MrpCreateInput, MrpListRequest, MrpRecord, MrpUpdateInput } from "./types";

const records = new Map<string, MrpRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const mrpRepository = {
  async list(request: MrpListRequest): Promise<PageResult<MrpRecord>> {
  ...
  async create(input: MrpCreateInput, actorId = "system"): Promise<MrpRecord> {
    const timestamp = now();
  ...
  async update(tenantId: string, id: string, input: MrpUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: MrpRecord = {
      ...current,
      ...input,
