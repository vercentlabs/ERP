...
import type { CapacityPlanningCreateInput, CapacityPlanningListRequest, CapacityPlanningRecord, CapacityPlanningUpdateInput } from "./types";

const records = new Map<string, CapacityPlanningRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const capacityPlanningRepository = {
  async list(request: CapacityPlanningListRequest): Promise<PageResult<CapacityPlanningRecord>> {
  ...
  async create(input: CapacityPlanningCreateInput, actorId = "system"): Promise<CapacityPlanningRecord> {
    const timestamp = now();
  ...
  async update(tenantId: string, id: string, input: CapacityPlanningUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: CapacityPlanningRecord = {
      ...current,
      ...input,
  ...
  async save(record: CapacityPlanningRecord) {
    records.set(record.id, record);
    return record;
  },
};
