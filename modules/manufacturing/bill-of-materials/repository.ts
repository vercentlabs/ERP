...
import type { BillOfMaterialsCreateInput, BillOfMaterialsListRequest, BillOfMaterialsRecord, BillOfMaterialsUpdateInput } from "./types";

const records = new Map<string, BillOfMaterialsRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const billOfMaterialsRepository = {
  async list(request: BillOfMaterialsListRequest): Promise<PageResult<BillOfMaterialsRecord>> {
  ...
  async create(input: BillOfMaterialsCreateInput, actorId = "system"): Promise<BillOfMaterialsRecord> {
    const timestamp = now();
    const record: BillOfMaterialsRecord = {
      id: createId(),
  ...
  async update(tenantId: string, id: string, input: BillOfMaterialsUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: BillOfMaterialsRecord = {
      ...current,
      ...input,
  ...
  async save(record: BillOfMaterialsRecord) {
    records.set(record.id, record);
    return record;
  },
};
