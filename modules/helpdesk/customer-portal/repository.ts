...
import type { CustomerPortalCreateInput, CustomerPortalListRequest, CustomerPortalRecord, CustomerPortalUpdateInput } from "./types";

const records = new Map<string, CustomerPortalRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const customerPortalRepository = {
  async list(request: CustomerPortalListRequest): Promise<PageResult<CustomerPortalRecord>> {
  ...
  async create(input: CustomerPortalCreateInput, actorId = "system"): Promise<CustomerPortalRecord> {
    const timestamp = now();
  ...
  async update(tenantId: string, id: string, input: CustomerPortalUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: CustomerPortalRecord = {
      ...current,
      ...input,
  ...
  async save(record: CustomerPortalRecord) {
    records.set(record.id, record);
    return record;
  },
};
