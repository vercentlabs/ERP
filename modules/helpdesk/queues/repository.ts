...
import type { QueuesCreateInput, QueuesListRequest, QueuesRecord, QueuesUpdateInput } from "./types";

const records = new Map<string, QueuesRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const queuesRepository = {
  async list(request: QueuesListRequest): Promise<PageResult<QueuesRecord>> {
  ...
  async create(input: QueuesCreateInput, actorId = "system"): Promise<QueuesRecord> {
    const timestamp = now();
    const record: QueuesRecord = {
      id: createId(),
  ...
  async update(tenantId: string, id: string, input: QueuesUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: QueuesRecord = {
      ...current,
      ...input,
  ...
  async save(record: QueuesRecord) {
    records.set(record.id, record);
    return record;
  },
};
