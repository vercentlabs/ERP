...
import type { EscalationsCreateInput, EscalationsListRequest, EscalationsRecord, EscalationsUpdateInput } from "./types";

const records = new Map<string, EscalationsRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const escalationsRepository = {
  async list(request: EscalationsListRequest): Promise<PageResult<EscalationsRecord>> {
  ...
  async create(input: EscalationsCreateInput, actorId = "system"): Promise<EscalationsRecord> {
    const timestamp = now();
    const record: EscalationsRecord = {
      id: createId(),
  ...
  async update(tenantId: string, id: string, input: EscalationsUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: EscalationsRecord = {
      ...current,
      ...input,
  ...
  async save(record: EscalationsRecord) {
    records.set(record.id, record);
    return record;
  },
};
