...
import type { TicketsCreateInput, TicketsListRequest, TicketsRecord, TicketsUpdateInput } from "./types";

const records = new Map<string, TicketsRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const ticketsRepository = {
  async list(request: TicketsListRequest): Promise<PageResult<TicketsRecord>> {
  ...
  async create(input: TicketsCreateInput, actorId = "system"): Promise<TicketsRecord> {
    const timestamp = now();
    const record: TicketsRecord = {
      id: createId(),
  ...
  async update(tenantId: string, id: string, input: TicketsUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: TicketsRecord = {
      ...current,
      ...input,
  ...
  async save(record: TicketsRecord) {
    records.set(record.id, record);
    return record;
  },
};
