...
import type { KnowledgeBaseCreateInput, KnowledgeBaseListRequest, KnowledgeBaseRecord, KnowledgeBaseUpdateInput } from "./types";
...
export const knowledgeBaseRepository = {
  async list(request: KnowledgeBaseListRequest): Promise<PageResult<KnowledgeBaseRecord>> {
  ...
  async create(input: KnowledgeBaseCreateInput, actorId = "system"): Promise<KnowledgeBaseRecord> {
    const timestamp = now();
  ...
  async update(tenantId: string, id: string, input: KnowledgeBaseUpdateInput, actorId = "system") {
  ...
  async save(record: KnowledgeBaseRecord) {
    records.set(record.id, record);
    return record;
  },
};
