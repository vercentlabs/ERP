...
import type { SlasCreateInput, SlasListRequest, SlasRecord, SlasUpdateInput } from "./types";

const records = new Map<string, SlasRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const slasRepository = {
  async list(request: SlasListRequest): Promise<PageResult<SlasRecord>> {
  ...
  async create(input: SlasCreateInput, actorId = "system"): Promise<SlasRecord> {
    const timestamp = now();
    const record: SlasRecord = {
      id: createId(),
  ...
  async update(tenantId: string, id: string, input: SlasUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: SlasRecord = {
      ...current,
      ...input,
