...
import type { EntitlementsCreateInput, EntitlementsListRequest, EntitlementsRecord, EntitlementsUpdateInput } from "./types";

const records = new Map<string, EntitlementsRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const entitlementsRepository = {
  async list(request: EntitlementsListRequest): Promise<PageResult<EntitlementsRecord>> {
  ...
  async create(input: EntitlementsCreateInput, actorId = "system"): Promise<EntitlementsRecord> {
    const timestamp = now();
    const record: EntitlementsRecord = {
      id: createId(),
  ...
  async update(tenantId: string, id: string, input: EntitlementsUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: EntitlementsRecord = {
      ...current,
      ...input,
  ...
  async save(record: EntitlementsRecord) {
    records.set(record.id, record);
    return record;
  },
};
