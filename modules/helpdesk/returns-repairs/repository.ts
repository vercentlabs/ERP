...
import type { ReturnsRepairsCreateInput, ReturnsRepairsListRequest, ReturnsRepairsRecord, ReturnsRepairsUpdateInput } from "./types";

const records = new Map<string, ReturnsRepairsRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const returnsRepairsRepository = {
  async list(request: ReturnsRepairsListRequest): Promise<PageResult<ReturnsRepairsRecord>> {
  ...
  async create(input: ReturnsRepairsCreateInput, actorId = "system"): Promise<ReturnsRepairsRecord> {
    const timestamp = now();
    const record: ReturnsRepairsRecord = {
      id: createId(),
  ...
  async update(tenantId: string, id: string, input: ReturnsRepairsUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: ReturnsRepairsRecord = {
      ...current,
      ...input,
  ...
  async save(record: ReturnsRepairsRecord) {
    records.set(record.id, record);
    return record;
  },
};
