...
import type { SalesTargetsCreateInput, SalesTargetsListRequest, SalesTargetsRecord, SalesTargetsUpdateInput } from "./types";

const records = new Map<string, SalesTargetsRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const salesTargetsRepository = {
  async list(request: SalesTargetsListRequest): Promise<PageResult<SalesTargetsRecord>> {
  ...
  async create(input: SalesTargetsCreateInput, actorId = "system"): Promise<SalesTargetsRecord> {
    const timestamp = now();
    const record: SalesTargetsRecord = {
      id: createId(),
  ...
  async update(tenantId: string, id: string, input: SalesTargetsUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: SalesTargetsRecord = {
      ...current,
      ...input,
  ...
  async save(record: SalesTargetsRecord) {
    records.set(record.id, record);
    return record;
  },
};
