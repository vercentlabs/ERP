...
import type { ReturnsCreateInput, ReturnsListRequest, ReturnsRecord, ReturnsUpdateInput } from "./types";

const records = new Map<string, ReturnsRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const returnsRepository = {
  async list(request: ReturnsListRequest): Promise<PageResult<ReturnsRecord>> {
  ...
  async create(input: ReturnsCreateInput, actorId = "system"): Promise<ReturnsRecord> {
    const timestamp = now();
  ...
  async update(tenantId: string, id: string, input: ReturnsUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: ReturnsRecord = {
      ...current,
      ...input,
  ...
  async save(record: ReturnsRecord) {
    records.set(record.id, record);
    return record;
  },
};
