...
import type { PriceBooksCreateInput, PriceBooksListRequest, PriceBooksRecord, PriceBooksUpdateInput } from "./types";

const records = new Map<string, PriceBooksRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const priceBooksRepository = {
  async list(request: PriceBooksListRequest): Promise<PageResult<PriceBooksRecord>> {
  ...
  async create(input: PriceBooksCreateInput, actorId = "system"): Promise<PriceBooksRecord> {
    const timestamp = now();
    const record: PriceBooksRecord = {
      id: createId(),
  ...
  async update(tenantId: string, id: string, input: PriceBooksUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: PriceBooksRecord = {
      ...current,
      ...input,
  ...
  async save(record: PriceBooksRecord) {
    records.set(record.id, record);
    return record;
  },
};
