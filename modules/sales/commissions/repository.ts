...
import type { CommissionsCreateInput, CommissionsListRequest, CommissionsRecord, CommissionsUpdateInput } from "./types";

const records = new Map<string, CommissionsRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const commissionsRepository = {
  async list(request: CommissionsListRequest): Promise<PageResult<CommissionsRecord>> {
  ...
  async create(input: CommissionsCreateInput, actorId = "system"): Promise<CommissionsRecord> {
    const timestamp = now();
    const record: CommissionsRecord = {
      id: createId(),
  ...
  async update(tenantId: string, id: string, input: CommissionsUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: CommissionsRecord = {
      ...current,
      ...input,
  ...
  async save(record: CommissionsRecord) {
    records.set(record.id, record);
    return record;
  },
};
