...
import type { ForecastingCreateInput, ForecastingListRequest, ForecastingRecord, ForecastingUpdateInput } from "./types";

const records = new Map<string, ForecastingRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const forecastingRepository = {
  async list(request: ForecastingListRequest): Promise<PageResult<ForecastingRecord>> {
  ...
  async create(input: ForecastingCreateInput, actorId = "system"): Promise<ForecastingRecord> {
    const timestamp = now();
    const record: ForecastingRecord = {
      id: createId(),
  ...
  async update(tenantId: string, id: string, input: ForecastingUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: ForecastingRecord = {
      ...current,
      ...input,
  ...
  async save(record: ForecastingRecord) {
    records.set(record.id, record);
    return record;
  },
};
