...
import type { DiscountsCreateInput, DiscountsListRequest, DiscountsRecord, DiscountsUpdateInput } from "./types";

const records = new Map<string, DiscountsRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const discountsRepository = {
  async list(request: DiscountsListRequest): Promise<PageResult<DiscountsRecord>> {
  ...
  async create(input: DiscountsCreateInput, actorId = "system"): Promise<DiscountsRecord> {
    const timestamp = now();
    const record: DiscountsRecord = {
      id: createId(),
  ...
  async update(tenantId: string, id: string, input: DiscountsUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: DiscountsRecord = {
      ...current,
      ...input,
  ...
  async save(record: DiscountsRecord) {
    records.set(record.id, record);
    return record;
  },
};
