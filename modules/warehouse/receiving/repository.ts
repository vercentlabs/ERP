...
import type { ReceivingCreateInput, ReceivingListRequest, ReceivingRecord, ReceivingUpdateInput } from "./types";

const records = new Map<string, ReceivingRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const receivingRepository = {
  async list(request: ReceivingListRequest): Promise<PageResult<ReceivingRecord>> {
  ...
  async create(input: ReceivingCreateInput, actorId = "system"): Promise<ReceivingRecord> {
    const timestamp = now();
  ...
  async update(tenantId: string, id: string, input: ReceivingUpdateInput, actorId = "system") {
  ...
  async save(record: ReceivingRecord) {
    records.set(record.id, record);
    return record;
  },
};
