...
import type { ShippingCreateInput, ShippingListRequest, ShippingRecord, ShippingUpdateInput } from "./types";

const records = new Map<string, ShippingRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const shippingRepository = {
  async list(request: ShippingListRequest): Promise<PageResult<ShippingRecord>> {
  ...
  async create(input: ShippingCreateInput, actorId = "system"): Promise<ShippingRecord> {
    const timestamp = now();
  ...
  async update(tenantId: string, id: string, input: ShippingUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: ShippingRecord = {
      ...current,
      ...input,
  ...
  async save(record: ShippingRecord) {
    records.set(record.id, record);
    return record;
  },
};
