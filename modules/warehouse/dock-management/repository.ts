...
import type { DockManagementCreateInput, DockManagementListRequest, DockManagementRecord, DockManagementUpdateInput } from "./types";

const records = new Map<string, DockManagementRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const dockManagementRepository = {
  async list(request: DockManagementListRequest): Promise<PageResult<DockManagementRecord>> {
  ...
  async create(input: DockManagementCreateInput, actorId = "system"): Promise<DockManagementRecord> {
    const timestamp = now();
    const record: DockManagementRecord = {
      id: createId(),
  ...
  async update(tenantId: string, id: string, input: DockManagementUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: DockManagementRecord = {
      ...current,
      ...input,
  ...
  async save(record: DockManagementRecord) {
    records.set(record.id, record);
    return record;
  },
};
