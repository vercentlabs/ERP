...
import type { MobileScanningCreateInput, MobileScanningListRequest, MobileScanningRecord, MobileScanningUpdateInput } from "./types";

const records = new Map<string, MobileScanningRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const mobileScanningRepository = {
  async list(request: MobileScanningListRequest): Promise<PageResult<MobileScanningRecord>> {
  ...
  async create(input: MobileScanningCreateInput, actorId = "system"): Promise<MobileScanningRecord> {
    const timestamp = now();
    const record: MobileScanningRecord = {
      id: createId(),
  ...
  async update(tenantId: string, id: string, input: MobileScanningUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: MobileScanningRecord = {
      ...current,
      ...input,
  ...
  async save(record: MobileScanningRecord) {
    records.set(record.id, record);
    return record;
  },
};
