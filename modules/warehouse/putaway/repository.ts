...
import type { PutawayCreateInput, PutawayListRequest, PutawayRecord, PutawayUpdateInput } from "./types";

const records = new Map<string, PutawayRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const putawayRepository = {
  async list(request: PutawayListRequest): Promise<PageResult<PutawayRecord>> {
  ...
  async create(input: PutawayCreateInput, actorId = "system"): Promise<PutawayRecord> {
    const timestamp = now();
  ...
  async update(tenantId: string, id: string, input: PutawayUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: PutawayRecord = {
      ...current,
      ...input,
  ...
  async save(record: PutawayRecord) {
    records.set(record.id, record);
    return record;
  },
};
