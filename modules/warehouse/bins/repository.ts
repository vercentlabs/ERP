...
import type { BinsCreateInput, BinsListRequest, BinsRecord, BinsUpdateInput } from "./types";

const records = new Map<string, BinsRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const binsRepository = {
  async list(request: BinsListRequest): Promise<PageResult<BinsRecord>> {
  ...
  async create(input: BinsCreateInput, actorId = "system"): Promise<BinsRecord> {
    const timestamp = now();
    const record: BinsRecord = {
      id: createId(),
  ...
  async update(tenantId: string, id: string, input: BinsUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: BinsRecord = {
      ...current,
      ...input,
