...
import type { PackingCreateInput, PackingListRequest, PackingRecord, PackingUpdateInput } from "./types";

const records = new Map<string, PackingRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const packingRepository = {
  async list(request: PackingListRequest): Promise<PageResult<PackingRecord>> {
  ...
  async create(input: PackingCreateInput, actorId = "system"): Promise<PackingRecord> {
    const timestamp = now();
  ...
  async update(tenantId: string, id: string, input: PackingUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: PackingRecord = {
      ...current,
      ...input,
  ...
  async save(record: PackingRecord) {
    records.set(record.id, record);
    return record;
  },
};
