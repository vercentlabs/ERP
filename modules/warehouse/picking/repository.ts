...
import type { PickingCreateInput, PickingListRequest, PickingRecord, PickingUpdateInput } from "./types";

const records = new Map<string, PickingRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const pickingRepository = {
  async list(request: PickingListRequest): Promise<PageResult<PickingRecord>> {
  ...
  async create(input: PickingCreateInput, actorId = "system"): Promise<PickingRecord> {
    const timestamp = now();
  ...
  async update(tenantId: string, id: string, input: PickingUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: PickingRecord = {
      ...current,
      ...input,
  ...
  async save(record: PickingRecord) {
    records.set(record.id, record);
    return record;
  },
};
