...
import type { ApparelCreateInput, ApparelListRequest, ApparelRecord, ApparelUpdateInput } from "./types";

const records = new Map<string, ApparelRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const apparelRepository = {
  async list(request: ApparelListRequest): Promise<PageResult<ApparelRecord>> {
  ...
  async create(input: ApparelCreateInput, actorId = "system"): Promise<ApparelRecord> {
    const timestamp = now();
    const record: ApparelRecord = {
      id: createId(),
  ...
  async update(tenantId: string, id: string, input: ApparelUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: ApparelRecord = {
      ...current,
      ...input,
  ...
  async save(record: ApparelRecord) {
    records.set(record.id, record);
    return record;
  },
};
