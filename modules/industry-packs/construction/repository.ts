...
import type { ConstructionCreateInput, ConstructionListRequest, ConstructionRecord, ConstructionUpdateInput } from "./types";

const records = new Map<string, ConstructionRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const constructionRepository = {
  async list(request: ConstructionListRequest): Promise<PageResult<ConstructionRecord>> {
  ...
  async create(input: ConstructionCreateInput, actorId = "system"): Promise<ConstructionRecord> {
    const timestamp = now();
  ...
  async update(tenantId: string, id: string, input: ConstructionUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: ConstructionRecord = {
      ...current,
      ...input,
  ...
  async save(record: ConstructionRecord) {
    records.set(record.id, record);
    return record;
  },
};
