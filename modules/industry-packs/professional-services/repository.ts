...
import type { ProfessionalServicesCreateInput, ProfessionalServicesListRequest, ProfessionalServicesRecord, ProfessionalServicesUpdateInput } from "./types";

const records = new Map<string, ProfessionalServicesRecord>();
...
export const professionalServicesRepository = {
  async list(request: ProfessionalServicesListRequest): Promise<PageResult<ProfessionalServicesRecord>> {
  ...
  async create(input: ProfessionalServicesCreateInput, actorId = "system"): Promise<ProfessionalServicesRecord> {
    const timestamp = now();
  ...
  async update(tenantId: string, id: string, input: ProfessionalServicesUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: ProfessionalServicesRecord = {
      ...current,
      ...input,
  ...
  async save(record: ProfessionalServicesRecord) {
    records.set(record.id, record);
    return record;
  },
};
