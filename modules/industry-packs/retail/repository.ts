...
import type { RetailCreateInput, RetailListRequest, RetailRecord, RetailUpdateInput } from "./types";

const records = new Map<string, RetailRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const retailRepository = {
  async list(request: RetailListRequest): Promise<PageResult<RetailRecord>> {
  ...
  async create(input: RetailCreateInput, actorId = "system"): Promise<RetailRecord> {
    const timestamp = now();
    const record: RetailRecord = {
      id: createId(),
  ...
  async update(tenantId: string, id: string, input: RetailUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: RetailRecord = {
      ...current,
      ...input,
  ...
  async save(record: RetailRecord) {
    records.set(record.id, record);
    return record;
  },
};
