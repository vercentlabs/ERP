...
import type { SoftwareSaasCreateInput, SoftwareSaasListRequest, SoftwareSaasRecord, SoftwareSaasUpdateInput } from "./types";

const records = new Map<string, SoftwareSaasRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const softwareSaasRepository = {
  async list(request: SoftwareSaasListRequest): Promise<PageResult<SoftwareSaasRecord>> {
  ...
  async create(input: SoftwareSaasCreateInput, actorId = "system"): Promise<SoftwareSaasRecord> {
    const timestamp = now();
    const record: SoftwareSaasRecord = {
      id: createId(),
  ...
  async update(tenantId: string, id: string, input: SoftwareSaasUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: SoftwareSaasRecord = {
      ...current,
      ...input,
  ...
  async save(record: SoftwareSaasRecord) {
    records.set(record.id, record);
    return record;
  },
};
