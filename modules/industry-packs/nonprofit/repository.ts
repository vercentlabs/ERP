...
import type { NonprofitCreateInput, NonprofitListRequest, NonprofitRecord, NonprofitUpdateInput } from "./types";

const records = new Map<string, NonprofitRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const nonprofitRepository = {
  async list(request: NonprofitListRequest): Promise<PageResult<NonprofitRecord>> {
  ...
  async create(input: NonprofitCreateInput, actorId = "system"): Promise<NonprofitRecord> {
    const timestamp = now();
  ...
  async update(tenantId: string, id: string, input: NonprofitUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: NonprofitRecord = {
      ...current,
      ...input,
  ...
  async save(record: NonprofitRecord) {
    records.set(record.id, record);
    return record;
  },
};
