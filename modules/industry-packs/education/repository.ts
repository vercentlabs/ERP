...
import type { EducationCreateInput, EducationListRequest, EducationRecord, EducationUpdateInput } from "./types";
...
export const educationRepository = {
  async list(request: EducationListRequest): Promise<PageResult<EducationRecord>> {
  ...
  async create(input: EducationCreateInput, actorId = "system"): Promise<EducationRecord> {
    const timestamp = now();
  ...
  async update(tenantId: string, id: string, input: EducationUpdateInput, actorId = "system") {
  ...
  async save(record: EducationRecord) {
    records.set(record.id, record);
    return record;
  },
};
