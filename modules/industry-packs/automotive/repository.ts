...
import type { AutomotiveCreateInput, AutomotiveListRequest, AutomotiveRecord, AutomotiveUpdateInput } from "./types";

const records = new Map<string, AutomotiveRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const automotiveRepository = {
  async list(request: AutomotiveListRequest): Promise<PageResult<AutomotiveRecord>> {
  ...
  async create(input: AutomotiveCreateInput, actorId = "system"): Promise<AutomotiveRecord> {
    const timestamp = now();
    const record: AutomotiveRecord = {
      id: createId(),
  ...
  async update(tenantId: string, id: string, input: AutomotiveUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: AutomotiveRecord = {
      ...current,
      ...input,
  ...
  async save(record: AutomotiveRecord) {
    records.set(record.id, record);
    return record;
  },
};
