...
import type { HealthcareCreateInput, HealthcareListRequest, HealthcareRecord, HealthcareUpdateInput } from "./types";

const records = new Map<string, HealthcareRecord>();

const now = () => new Date().toISOString();
const createId = () => crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export const healthcareRepository = {
  async list(request: HealthcareListRequest): Promise<PageResult<HealthcareRecord>> {
  ...
  async create(input: HealthcareCreateInput, actorId = "system"): Promise<HealthcareRecord> {
    const timestamp = now();
    const record: HealthcareRecord = {
      id: createId(),
  ...
  async update(tenantId: string, id: string, input: HealthcareUpdateInput, actorId = "system") {
    const current = await this.getById(tenantId, id);
    if (!current) return undefined;
    const updated: HealthcareRecord = {
      ...current,
      ...input,
  ...
  async save(record: HealthcareRecord) {
    records.set(record.id, record);
    return record;
  },
};
