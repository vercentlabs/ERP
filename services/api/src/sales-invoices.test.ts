...
function fakeInvoiceRepository(): SalesInvoiceRepository {
  ...
  return {
    ...
    async applySalesInvoiceCreditAllocation(tenantId, id, allocatedAmount) { const current = await this.getSalesInvoiceById(tenantId, id); if (!current || current.status !== "ISSUED" || current.accountingStatus !== "POSTED" || current.amountDue < allocatedAmount) return undefined; const amountDue = Math.round((current.amountDue - allocatedAmount + Number.EPSILON) * 100) / 100; const updated: SalesInvoiceRecord = { ...current, amountDue, paymentStatus: amountDue === 0 ? "PAID" : current.amountPaid > 0 ? "PARTIALLY_PAID" : "UNPAID", updatedAt: now() }; records.set(id, updated); return updated; },
