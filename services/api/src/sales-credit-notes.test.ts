...
function fakeCreditRepo(): SalesCreditNoteRepository {
  ...
  return {
    ...
    async applyCreditNoteAllocation(tenantId, id, amount) { const current = await this.getSalesCreditNoteById(tenantId, id); if (!current || current.status !== "POSTED" || current.availableAmount < amount) return undefined; const updated = { ...current, allocatedAmount: current.allocatedAmount + amount, availableAmount: Math.round((current.availableAmount - amount + Number.EPSILON) * 100) / 100, updatedAt: now() }; records.set(id, updated); return updated; },
    async applyCreditNoteRefund(tenantId, id, amount) { const current = await this.getSalesCreditNoteById(tenantId, id); if (!current || current.status !== "POSTED" || current.availableAmount < amount) return undefined; const updated = { ...current, refundedAmount: current.refundedAmount + amount, availableAmount: Math.round((current.availableAmount - amount + Number.EPSILON) * 100) / 100, updatedAt: now() }; records.set(id, updated); return updated; },
    async getCreditNoteAvailableAmount(tenantId, id) { return (await this.getSalesCreditNoteById(tenantId, id))?.availableAmount ?? 0; },
